...
| 攻击 | 全部 `combat.attacks[]` | 保留两组原始武器的启用态、平均伤害、射程、攻击间隔、伤害点、武器类型和空中/地面/建筑目标层；目标切换时按稳定 slot 选武器 |
...
| 科技/UI | Upgrade 逐级名称、说明、图标、费用、时间、最大等级、`requirements` | 当前 15 项人族研究全部进入目录；原始建筑/科技前置编译为通用 requirement，命令卡显示原版中文数据 |
...
攻击目标层和当前人族多武器已经进入确定性运行时。尚未接入的主要战斗字段是 War3
完整攻防类型矩阵、普通武器自身的区域/溅射参数、最小射程、夜间视野和单位护甲科技；
城镇大厅→主城→城堡的正式升级命令也仍需独立建筑升级协议，不能伪装成农民建造配方。

## 验证

1. 构建 C# 项目，确保 nullable 与 warnings-as-errors 全部通过。
2. 运行 `war3_rts/War3Rts.tscn`；日志应同时包含 `WAR3_GAMEPLAY_DATA` 和 `WAR3_OBJECT_DATA`，后者应为 `technology_applied=15 technology_fallbacks=0`。
...
5. 使用现有 `--war3-rts-smoke` 冒烟流程；`data_integration=True` 会验证 manifest、可玩技能/升级引用、农民视野换算、飞行单位 Elevated 视野和科技消费点 ID。
