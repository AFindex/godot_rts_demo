...
v5 的战斗 Profile 包含 Armor、属性位集、AttacksPerVolley、BonusVs/BonusDamage、基础/加成每级武器升级量、ProjectileSpeed，以及 CanMoveDuringWindup/CanMoveDuringCooldown。速度为 0 表示瞬时命中，大于 0 使用确定性跟踪投射物；两个移动字段默认 false，只有明确设计为移动射击的武器才在 AttackMove 攻击周期中恢复路线。属性支持 Light、Armored、Biological、Mechanical、Structure、Massive 的合法组合，攻击段数限制为 1～32。
...
当前规范 Hash 为 `722A502E8B6F3F33`。`ProductionCatalogDiff` 分别统计 Unit Type
...
Production Command Log v7、Replay Package/Hot Snapshot v16 和 State Hash v17 保存完整战斗字段、武器移动约束与活跃投射物。Replay Package 的 Train 命令保存当时解析后的完整 Recipe/Unit Type/Requirements，因此之后修改
Resource 不会篡改已有录像。
