...
`terrain-dynamic` 必须取得 `RTS_TERRAIN_DYNAMIC_DEMO_PASS`：Large 建筑存在时第一波走远坡 Choke 1，拆除后第二波走近坡 Choke 0；两波全部到达、无非法穿崖、无重叠，并且两次变化均为 Chunk 增量更新、完整重建为 0。

`terrain-presets` 必须从已保存的 `.tres` 加载全部 12 张地图，逐张显示稳定元数据，并取得 `RTS_TERRAIN_PRESETS_DEMO_PASS`；缺失、损坏、过期哈希或坡道数不匹配都会在录像前失败。
