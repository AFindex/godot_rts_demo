...
实现遵循 Blizzard 经典版说明中的选择规则：一次最多选择 12 个单位；同类型单位组成子组；英雄优先，其次是有主动技能的单位，普通战斗单位排后；英雄子组按等级排序。当前子组的迷你头像会放大并以金色描边，负面状态单位以红框提示，每个头像都有独立血条和可选蓝条。
...
- 框选和 Shift 追加都会执行 12 单位上限，信息区使用 4×3 头像布局。
