...
- 127 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
- 168 个 C# 源文件。
- 约 58,791 行 C#（按当前 `src/**/*.cs` 统计）。
- 127 个黑盒场景。
- AV1/WebM 采用固定 AV1/WebM 规范保存；当前里程碑新增场景有独立录像。
- 当前温热 Release 1000 单位移动 P95：18.209ms（预算 16.67ms），未通过。
- 当前五档分配门禁仍全部未通过；预算没有放宽，仓库提交的详细原始结果见 `benchmark_results/latest.json`。这两项作为独立性能 TODO，不影响已经通过的业务正确性结论。
