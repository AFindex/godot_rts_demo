...
- 90 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
- 124 个 C# 源文件。
- 约 39,200 行 C#（按 `src/**/*.cs` 统计）。
- 90 个黑盒场景。
- AV1/WebM 规范录像覆盖全部 90 个当前逻辑场景。
- Release 1000 单位移动 P95：约 8.98ms。
