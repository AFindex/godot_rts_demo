...
- 68 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem 或队列内部数组。
...
- 83 个 C# 源文件。
- 约 21,864 行 C#（按 `src/**/*.cs` 统计）。
- 68 个黑盒场景。
- 覆盖 68 个逻辑场景的规范测试录像。
- Release 1000 单位移动 P95：约 12.66ms。
...
|---:|---:|---:|---:|---:|---:|
| 256 | 0.98ms | 1.29ms | 0.744ms | 4ms | 27B |
| 512 | 3.69ms | 4.72ms | 1.378ms | 12.5ms | 182B |
| 1000 | 9.10ms | 12.66ms | 1.743ms | 16.67ms | 461B |

双方持续 AttackMove 的活跃战斗门槛：

| 总单位数 | 平均 Tick | P95 | Hash 平均 | 战斗阶段平均 | 当前门槛 | 分配/Tick |
|---:|---:|---:|---:|---:|---:|---:|
| 128 | 1.42ms | 1.91ms | 0.158ms | 0.51ms | 4ms | 2.3KB |
| 256 | 3.57ms | 5.04ms | 0.753ms | 1.71ms | 8ms | 4.0KB |
...
- 1000 单位 P95 继续低于 16.67ms，分配低于 1KB/Tick。（12.66ms / 461B）
...
移动内核、基础操作表现、S9 数据工作流与 S11-B 经济确定性格式均已形成 Demo 闭环。下一轮进入 S11-C 正式建造系统：先建立数据驱动建筑定义、建造实体生命周期和经济事务，再接施工策略、取消退款、Refinery 节点绑定、受击/死亡与回放快照。生产、科技、胜负和脚本 AI 继续按依赖顺序后推；导航只修复实际玩法暴露的阻塞问题。
