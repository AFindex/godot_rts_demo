...
| S3 群体抵达 | 大部分完成 | 唯一槽位、Hungarian 分配、跨命令槽位预留、局部换槽、终点驻留历史、主动 Yielding、唯一 Overflow | 局部多单位匹配、SlotDepth、终点专用 Steering、编队形状保持 |
...
|---:|---:|---:|---:|---:|
| 256 | 1.03ms | 1.36ms | 4ms | 27B |
| 512 | 3.59ms | 4.51ms | 12.5ms | 182B |
| 1000 | 7.22ms | 8.84ms | 16.67ms | 461B |
...
“外围先就位、内部槽位后到”的永久卡死已由 V2 前两层解决：专用场景外圈 48/48、内部 32/32，触发 8 次主动让路，最终 0 Moving、0 重叠、最大误差 7.2px。Overflow 从第一层的 32 次降到 31 次，已有内部单位实际穿过临时打开的通道；但局部多单位重匹配和 SlotDepth 尚未完成，因此 A2 仍未关闭。

TODO：

- 终点收敛专用 Steering 权重。
- 8～24 单位局部重新匹配，支持三单位以上循环换槽。
- SlotDepth 和进入方向排序，让深层槽位优先通过。
- 增加靠墙、角落、混合半径和动态建筑目标场景。
...
### 下一步 A2：终点协作收敛 V2（前两层完成，继续为当前优先级）
...
2. SlotDepth 和进入方向排序，让深层槽位优先进入。
3. 靠墙、角落、混合半径和动态建筑目标处理。
...
- 1000 单位 P95 继续低于 16.67ms，分配低于 1KB/Tick。（8.84ms / 461B）
