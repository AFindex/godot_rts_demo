...
- 86 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
- 120 个 C# 源文件。
- 约 37,600 行 C#（按 `src/**/*.cs` 统计）。
- 86 个黑盒场景。
- AV1/WebM 规范录像覆盖全部 86 个当前逻辑场景。
- Release 1000 单位移动 P95：约 10.21ms。
...
| 操作层 | J1 完成 | Shift 跨域任务、混合选择子组、快照命令卡、Control Group、SmartCommand、相机、Minimap | 建筑混合编组、Alt 操作、目标模式命令卡和最终皮肤 |
...
| S11 实际 RTS 玩法 | J1 完成 | 双资源、建筑/生产/科技、扩张、视野/胜负、双 AI、跨域 Shift、混合选择与命令卡、Package/Hot v12、Hash v13 | J2 目标模式命令卡和混合编组 |
...
- 建筑混合 Control Group 和 Alt 移出/窃取。
