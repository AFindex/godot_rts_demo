...
- 114 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
D1 至此收口。后续不继续扩展动态外交或共享资源；优先级回到 E0 当前 SC2 客户端实机矩阵，确认己方/盟友 Hold、Harvest、可见敌军等施工动作。扫描、Cloak/Burrow 能力开关、能量和种族例外仍按真实内容需求开启。
