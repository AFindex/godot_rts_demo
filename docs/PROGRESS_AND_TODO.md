...
- 98 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
- 135 个 C# 源文件。
- 约 42,268 行 C#（按 `src/**/*.cs` 统计）。
- 98 个黑盒场景。
- AV1/WebM 规范录像覆盖全部 98 个当前逻辑场景。
- Release 1000 单位移动 P95：约 8.43ms。
...
| S11 实际 RTS 玩法 | J2b2b + S8-E5a 完成 | 双资源、建筑/生产/科技、扩张、视野/胜负、双 AI、完整操作、弹道/移动射击/目标评分/接触优先级、Package/Hot v17、Hash v18 | 后续战斗只按实际失败推进 |
