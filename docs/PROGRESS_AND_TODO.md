...
- 29 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering 或 UnitStore 内部状态。
...
| S3 群体抵达 | 大部分完成 | 唯一槽位、Hungarian 分配、跨命令槽位预留、局部换槽、终点驻留历史、唯一 Overflow | 局部多单位匹配、主动 Yielding、终点专用 Steering、编队形状保持 |
...
|---:|---:|---:|---:|---:|
| 256 | 0.97ms | 1.37ms | 4ms | 27B |
| 512 | 3.49ms | 4.07ms | 12.5ms | 182B |
| 1000 | 6.82ms | 7.64ms | 16.67ms | 461B |
...
32 人以上编队会交换局部错位槽位，并在外圈封闭或终点驻留超时后分配唯一 Overflow。固定场景中，80 单位密集编队已达到 80/80、最大误差 4.9px；192 单位达到 192/192、最大误差 3.7px。

“外围先就位、内部槽位后到”的永久卡死已由 V2 第一层解决：专用场景外圈 48/48、内部 32/32，最终 0 Moving、最大误差 7.5px。当前 Overflow 是安全兜底，但最终编队会比局部主动让路更松散，因此 A2 仍未全部关闭。

TODO：

- 对已到达单位降低软阻挡影响并支持临时 Yielding。
- 终点收敛专用 Steering 权重。
- 三单位循环换槽或局部小规模重新匹配。
- SlotDepth 和进入方向排序，让深层槽位优先通过。
...
### 下一步 A2：终点协作收敛 V2（第一层完成，继续为当前优先级）
...
1. 8～24 单位局部重新匹配，支持三单位以上循环换槽。
2. 已到达单位进入 SoftOccupied/Yielding，必要时临时让出通道并返回。
3. SlotDepth、进入方向和靠墙/角落目标处理。

性能约束：

- 只处理 32 人以上、目标附近且持续无进展的编队。
...
- 普通密集编队结果不退化。（80/80）
- 1000 单位 P95 继续低于 16.67ms，分配低于 1KB/Tick。（7.64ms / 461B）
