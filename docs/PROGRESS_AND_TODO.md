...
- 57 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore 或队列内部数组。
...
- 59 个 C# 源文件。
- 约 16,775 行 C#（按仓库源码统计）。
- 57 个黑盒场景。
- 覆盖 57 个逻辑场景的规范测试录像。
- Release 1000 单位移动 P95：约 9.38ms。
...
|---:|---:|---:|---:|---:|---:|
| 256 | 1.26ms | 1.80ms | 0.750ms | 4ms | 27B |
| 512 | 4.66ms | 6.34ms | 1.449ms | 12.5ms | 182B |
| 1000 | 7.57ms | 9.38ms | 1.587ms | 16.67ms | 461B |

双方持续 AttackMove 的活跃战斗门槛：

| 总单位数 | 平均 Tick | P95 | Hash 平均 | 战斗阶段平均 | 当前门槛 | 分配/Tick |
|---:|---:|---:|---:|---:|---:|---:|
| 128 | 1.26ms | 1.84ms | 0.136ms | 0.46ms | 4ms | 2.3KB |
| 256 | 3.45ms | 4.75ms | 0.660ms | 1.63ms | 8ms | 4.0KB |
...
- 57/57 黑盒回归、四段确定性/回放录像和独立 Hash 性能门槛。
...
- 1000 单位 P95 继续低于 16.67ms，分配低于 1KB/Tick。（9.38ms / 461B）
...
保存影响未来模拟的完整运行状态，并直接恢复路径游标、命令队列、战斗状态、狭口租约、恢复状态机和待处理请求，避免 E4.1 从 Tick 0 重演。外部仍沿用 checkpoint 与 Replay Package 契约，不把 SoA 数组暴露给业务层。暂不引入联机协议。
