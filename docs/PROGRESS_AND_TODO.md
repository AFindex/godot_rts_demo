...
- 91 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
- 126 个 C# 源文件。
- 约 39,500 行 C#（按 `src/**/*.cs` 统计）。
- 91 个黑盒场景。
- AV1/WebM 规范录像覆盖全部 91 个当前逻辑场景。
- Release 1000 单位移动 P95：约 7.53ms。
...
| S8 战斗移动 | E1 战斗事件完成 | AttackMove、战斗占位、Stop/Hold、重选敌、死亡/leash/路线恢复、固定容量起手/命中/摧毁事件流 | 伤害/护甲与科技修正、弹道、移动射击、复杂目标权重和推挤优先级 |
...
| S11 实际 RTS 玩法 | J2b2b 完成 | 双资源、建筑/生产/科技、扩张、视野/胜负、双 AI、跨域 Shift、完整目标模式、同类多建筑批量生产、Package/Hot v12、Hash v13 | 明确玩法缺口或表现增强 |
...
后续顺序：真实伤害/护甲与科技修正、弹道；再由实际玩法决定移动射击、复杂仇恨权重和友军/敌军推挤优先级。动画/音效表现已可直接消费事件合同。
...
下一段为 S8-E2 真实伤害合同：护甲、单位属性/武器加成、多段攻击与现有 Infantry Weapons 科技修正。完成稳定命中结果后再把弹道接到当前事件流。
