...
- 89 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
- 122 个 C# 源文件。
- 约 38,800 行 C#（按 `src/**/*.cs` 统计）。
- 89 个黑盒场景。
- AV1/WebM 规范录像覆盖全部 89 个当前逻辑场景。
- Release 1000 单位移动 P95：约 8.17ms。
...
| 操作层 | J2b2a 完成 | Shift 跨域任务、混合选择/编组、快照命令卡、Move/AttackMove/Rally/Build 目标模式、SmartCommand、相机、Minimap | 批量建筑动作、队列聚合和最终皮肤 |
...
| S11 实际 RTS 玩法 | J2b2a 完成 | 双资源、建筑/生产/科技、扩张、视野/胜负、双 AI、跨域 Shift、完整目标模式、Package/Hot v12、Hash v13 | J2b2b 多建筑批量生产与队列聚合 |
...
- 多建筑批量生产/队列聚合、tooltip、图标与最终 UI 皮肤。
- 同类型多建筑批量生产与队列聚合显示。
...
J2b2b 只剩同类型多建筑批量生产、取消策略和队列聚合显示；Build 不再继续扩张，除非实际玩法暴露明确问题。
