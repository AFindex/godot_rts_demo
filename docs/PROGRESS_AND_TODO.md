...
| S3 群体抵达 | 大部分完成 | 唯一槽位、Hungarian 分配、跨命令槽位预留、32 人以上编队局部换槽 | 备用槽位、终点专用 Steering、编队形状保持 |
...
32 人以上编队已经会在目标区域交换局部错位槽位。固定场景中，80 单位密集编队从 76 个到达、最远约 88.8px，改善到 79 个到达、最远约 17.5px；192 单位到达数从 182 提升到 187。

仍有更复杂干扰场景会留下少量距离槽位约 50～100px 的 Moving 单位，因此终点问题是“显著改善但未完全关闭”。
