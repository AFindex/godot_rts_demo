...
- 88 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
- 122 个 C# 源文件。
- 约 38,400 行 C#（按 `src/**/*.cs` 统计）。
- 88 个黑盒场景。
- AV1/WebM 规范录像覆盖全部 88 个当前逻辑场景。
- Release 1000 单位移动 P95：约 9.25ms。
...
| 操作层 | J2b1 完成 | Shift 跨域任务、混合选择/编组、快照命令卡、Move/AttackMove/Rally 目标模式、SmartCommand、相机、Minimap | Build 放置模式、批量建筑动作和最终皮肤 |
...
| S11 实际 RTS 玩法 | J2b1 完成 | 双资源、建筑/生产/科技、扩张、视野/胜负、双 AI、跨域 Shift、混合操作层、Package/Hot v12、Hash v13 | J2b2 Build 放置模式和批量建筑动作 |
...
- Build 目标放置模式、tooltip、图标与最终 UI 皮肤。
- 同类型多建筑批量生产与队列聚合显示。
