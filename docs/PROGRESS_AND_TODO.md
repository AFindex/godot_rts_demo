...
- 55 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore 或队列内部数组。
...
- 57 个 C# 源文件。
- 约 15,463 行 C#（按仓库源码统计）。
- 55 个黑盒场景。
- 覆盖 55 个逻辑场景的规范测试录像。
- Release 1000 单位移动 P95：约 9.94ms。
...
|---:|---:|---:|---:|---:|---:|
| 256 | 1.26ms | 1.93ms | 0.736ms | 4ms | 27B |
| 512 | 3.97ms | 5.27ms | 1.447ms | 12.5ms | 182B |
| 1000 | 7.53ms | 9.94ms | 1.277ms | 16.67ms | 461B |

双方持续 AttackMove 的活跃战斗门槛：

| 总单位数 | 平均 Tick | P95 | Hash 平均 | 战斗阶段平均 | 当前门槛 | 分配/Tick |
|---:|---:|---:|---:|---:|---:|---:|
| 128 | 1.43ms | 2.00ms | 0.167ms | 0.52ms | 4ms | 2.3KB |
| 256 | 3.65ms | 4.94ms | 0.855ms | 1.74ms | 8ms | 4.0KB |
...
- 55/55 黑盒回归、两段规范录像和独立 Hash 性能门槛。

TODO：

- Replay Package：地图/Profile/Bake 版本与 Hash、初始单位清单。
- 把动态建筑放置/移除纳入世界命令日志。
- 快照保存、恢复和从中间 Tick 快速跳转。
...
- 1000 单位 P95 继续低于 16.67ms，分配低于 1KB/Tick。（9.94ms / 461B）
...
- 55/55 黑盒场景通过，两段确定性录像保存。
- 详细格式、覆盖边界和指标见 `docs/COMMAND_REPLAY.md`。

### 下一步 E3：Replay Package 与动态世界命令

先把地图/Profile/Bake 的版本与稳定 Hash、初始单位生成清单打包成可验证的回放前置条件，再把建筑放置/移除纳入世界命令。目标是让回放不依赖测试夹具在代码中手工重建初始状态。完成后再做快照恢复，不提前进入联机协议或跨平台 Lockstep。
