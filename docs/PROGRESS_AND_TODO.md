...
- 60 个 C# 源文件。
- 约 17,475 行 C#（按仓库源码统计）。
- 59 个黑盒场景。
- 覆盖 59 个逻辑场景的规范测试录像。
- Release 1000 单位移动 P95：约 9.75ms。
...
|---:|---:|---:|---:|---:|---:|
| 256 | 1.10ms | 1.44ms | 0.764ms | 4ms | 27B |
| 512 | 4.76ms | 6.00ms | 1.787ms | 12.5ms | 182B |
| 1000 | 7.47ms | 9.75ms | 1.425ms | 16.67ms | 461B |

双方持续 AttackMove 的活跃战斗门槛：

| 总单位数 | 平均 Tick | P95 | Hash 平均 | 战斗阶段平均 | 当前门槛 | 分配/Tick |
|---:|---:|---:|---:|---:|---:|---:|
| 128 | 1.30ms | 1.67ms | 0.160ms | 0.46ms | 4ms | 2.3KB |
| 256 | 3.40ms | 4.46ms | 0.715ms | 1.61ms | 8ms | 4.0KB |
...
- 1000 单位 P95 继续低于 16.67ms，分配低于 1KB/Tick。（9.75ms / 461B）
...
把进程内深拷贝转换为版本化规范字节，提供严格反序列化、磁盘保存和载荷大小/耗时门槛。完成后确定性基础设施收口，不继续扩张联机功能，返回双击选择、相机和 Minimap 操作层。
