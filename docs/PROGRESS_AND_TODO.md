...
- 58 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore 或队列内部数组。
...
- 59 个 C# 源文件。
- 约 16,934 行 C#（按仓库源码统计）。
- 58 个黑盒场景。
- 覆盖 58 个逻辑场景的规范测试录像。
- Release 1000 单位移动 P95：约 11.13ms。
...
|---:|---:|---:|---:|---:|---:|
| 256 | 1.08ms | 1.41ms | 1.077ms | 4ms | 27B |
| 512 | 3.97ms | 5.08ms | 1.285ms | 12.5ms | 182B |
| 1000 | 8.65ms | 11.13ms | 1.853ms | 16.67ms | 461B |

双方持续 AttackMove 的活跃战斗门槛：

| 总单位数 | 平均 Tick | P95 | Hash 平均 | 战斗阶段平均 | 当前门槛 | 分配/Tick |
|---:|---:|---:|---:|---:|---:|---:|
| 128 | 1.29ms | 1.72ms | 0.144ms | 0.45ms | 4ms | 2.3KB |
| 256 | 3.69ms | 4.87ms | 0.917ms | 1.77ms | 8ms | 4.0KB |
...
- 58/58 黑盒回归、五段确定性/回放录像和独立 Hash 性能门槛。

TODO：

- 直接运行时快照保存/恢复，避免 checkpoint 从 Tick 0 重演。
...
- 1000 单位 P95 继续低于 16.67ms，分配低于 1KB/Tick。（11.13ms / 461B）
...
前置审计已完成：状态 Hash 升级到 v2，补齐动态建筑下一 ID，以及 `LastDirection / ClearTicks / BurstTicks / wait ticks` 等狭口私有未来态；狭口协调器已有显式运行态捕获/恢复边界，32 单位活跃交通 checkpoint 已验证。

仍需实现主体：保存并直接恢复 Unit/Combat SoA、路径游标、命令队列、动态占用、狭口租约、恢复状态机和待处理请求，避免 E4.1 从 Tick 0 重演。外部继续沿用 checkpoint 与 Replay Package 契约，不把 SoA 数组暴露给业务层。暂不引入联机协议。
