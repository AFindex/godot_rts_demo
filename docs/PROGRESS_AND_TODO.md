...
- 60 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore 或队列内部数组。
...
|---:|---:|---:|---:|---:|---:|
| 256 | 0.98ms | 1.20ms | 0.846ms | 4ms | 27B |
| 512 | 3.57ms | 4.33ms | 1.353ms | 12.5ms | 182B |
| 1000 | 7.49ms | 9.45ms | 1.331ms | 16.67ms | 461B |

双方持续 AttackMove 的活跃战斗门槛：

| 总单位数 | 平均 Tick | P95 | Hash 平均 | 战斗阶段平均 | 当前门槛 | 分配/Tick |
|---:|---:|---:|---:|---:|---:|---:|
| 128 | 1.50ms | 2.14ms | 0.154ms | 0.55ms | 4ms | 2.3KB |
| 256 | 3.60ms | 4.77ms | 0.643ms | 1.74ms | 8ms | 4.0KB |
...
- 1000 单位 P95 继续低于 16.67ms，分配低于 1KB/Tick。（9.45ms / 461B）
...
- 60/60 黑盒场景通过，规范录像已保存。

### 下一步 F2：Minimap

实现世界缩略图、单位/建筑标记、当前视口框、点击定位和右键 SmartCommand。复用 `OperationCameraController` 与现有命令解析，不建立第二套坐标或命令规则。
