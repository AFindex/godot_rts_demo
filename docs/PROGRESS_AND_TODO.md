...
- 85 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
- 117 个 C# 源文件。
- 约 37,000 行 C#（按 `src/**/*.cs` 统计）。
- 85 个黑盒场景。
- AV1/WebM 规范录像覆盖全部 85 个当前逻辑场景。
- Release 1000 单位移动 P95：约 9.03ms。
...
| 操作层 | 玩法上下文第二层完成 | Shift 跨域工人任务、Control Group、资源/施工 SmartCommand、选择、相机、解耦 Minimap | Alt 编组、混合子组、命令卡和 UI 皮肤 |
...
| S11 实际 RTS 玩法 | I2 完成 | 双资源、五类建筑、生产/Rally、科技、扩张、视野/权限、胜负、模块化双 AI、资源/施工上下文与跨域 Shift、Package/Hot v12、Hash v13 | 之后由明确玩法缺口驱动 |
