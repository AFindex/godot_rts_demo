...
- 56 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore 或队列内部数组。
...
|---:|---:|---:|---:|---:|---:|
| 256 | 1.01ms | 1.38ms | 0.652ms | 4ms | 27B |
| 512 | 3.65ms | 4.63ms | 1.331ms | 12.5ms | 182B |
| 1000 | 7.36ms | 9.18ms | 1.324ms | 16.67ms | 461B |

双方持续 AttackMove 的活跃战斗门槛：

| 总单位数 | 平均 Tick | P95 | Hash 平均 | 战斗阶段平均 | 当前门槛 | 分配/Tick |
|---:|---:|---:|---:|---:|---:|---:|
| 128 | 1.53ms | 3.39ms | 0.151ms | 0.69ms | 4ms | 2.3KB |
| 256 | 4.38ms | 7.01ms | 0.867ms | 2.13ms | 8ms | 4.0KB |
...
- 56/56 黑盒回归、三段确定性/回放录像和独立 Hash 性能门槛。
...
### 下一步 E4：版本化快照与中间 Tick 恢复

保存影响未来模拟的完整运行状态，并从 checkpoint 恢复后继续消费同一 Replay Package。验收重点是“原运行”和“从中间 Tick 恢复”的后半段周期 Hash 全部一致，同时明确不序列化表现层、选择框和调试状态。暂不引入联机协议。
