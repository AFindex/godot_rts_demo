...
- 128 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
- 176 个 C# 源文件。
- 约 61,151 行 C#（按当前 `src/**/*.cs` 统计）。
- 128 个黑盒场景。
- AV1/WebM 采用固定 AV1/WebM 规范保存；当前里程碑新增场景有独立录像。
- 当前温热 Release 256/512/1000 单位移动 P95：4.486/7.720/27.807ms（预算 4/12.5/16.67ms）；256 与 1000 单位时间门禁未通过。
- 当前五档分配门禁仍全部未通过；预算没有放宽，仓库提交的详细原始结果见 `benchmark_results/latest.json`。这些作为独立性能 TODO，不影响已经通过的业务正确性结论。
...
| S11 实际 RTS 玩法 | H3/J2b2b + S8-E5a + SC2-D2a 工程闭环完成 | 双资源、建筑/生产/科技、扩张、视野/胜负、主动潜地、双 AI 持续遭遇战、完整操作与战斗、Package v30/Hot v31/Hash v32 | 后续能力由遭遇战关卡暴露的缺口驱动 |
...
- 避让侧记忆，减少左右反复横跳。
- 三轮并行圆碰撞位置修正；拥挤或贴墙时再执行最多 18 轮、无冲突即提前结束的顺序残余修正，避免多邻居修正互相抵消后留下单位重叠。
...
- 权威地形是纯 C# 不可变 `TerrainMapSnapshot`；Godot Resource、3D 网格和输入拾取均为适配层。
- 同一格数据独立表达高度层、表面材质、移动种类、可建造、坡道、视野/菌毯阻挡，不用贴图颜色反推玩法。
- 地面圆不能跨越悬崖，可经相邻高度坡道穿越；浅水允许地面单位通过但稳定拒绝建筑。
- 版本化地形 Resource 已真实保存/重新加载，当前可玩地图哈希为 `9704B5847C5B2221`。
- Clearance Bake 格式 2 分别绑定导航哈希与地形哈希，三档静态连通数据读取地形约束。
- 3D Presenter 批量生成地表和崖壁，所有世界表现按地形高度落地且不反向影响模拟。
- 独立生产栈场景 `TerrainTraversal3D.tscn` 验证 18/18 绕崖经坡到达、0 次非法穿崖、浅水禁造、Bake 匹配和最小间距 16.00。
- T1 已完成；下一项是 T2 高低地视野、共享视野和交战规则。完整边界见 `docs/SC2_TERRAIN_IMPLEMENTATION_PLAN.md`。
...
- 单项失败不会中止其他录像。
- 当前仓库包含 179 个规范录像文件、102 个 manifest 和 178 个场景引用。
...
|---:|---:|---:|---:|---:|---:|
| 256 | 1.824ms | 4.486ms | 1.441ms | 4ms | 37.4KB |
| 512 | 5.426ms | 7.720ms | 1.491ms | 12.5ms | 79.5KB |
| 1000 | 13.022ms | 27.807ms | 2.649ms | 16.67ms | 154.4KB |

双方持续 AttackMove 的活跃战斗门槛：

| 总单位数 | 平均 Tick | P95 | Hash 平均 | 战斗阶段平均 | 当前门槛 | 分配/Tick |
|---:|---:|---:|---:|---:|---:|---:|
| 128 | 0.540ms | 0.999ms | 0.484ms | 0.037ms | 4ms | 11.5KB |
| 256 | 1.939ms | 2.814ms | 0.584ms | 0.087ms | 8ms | 22.7KB |

活跃追击会持续生成短路径，因此单独使用 8KB/Tick 分配门槛；非战斗移动仍保持 1KB/Tick 门槛。当前五档分配均超过门槛，256/1000 单位移动时间也超过门槛；表格记录失败，不代表已通过。
...
### BQ：移动、交互、自动战斗与采矿紧急稳定化（已完成）

- 右键己方矩形建筑不再把整组选中单位塞向四个面中点。每个单位从建筑几何中心沿“中心→单位”射线投影到扩张后的矩形边缘，因真实障碍不可达时才进入有界接近点回退；四个对角来向得到四个不同的斜向接近目标并全部抵达。
- 普通玩家 Move 仍使用编队槽位；采矿、返矿和施工的系统派生移动改用精确业务接近点，禁止单单位槽位分配器二次偏移矩形 DropOff/施工接近点。默认 12 个开局农民改为一次批量分矿后下发 Gather，不再用索引取模伪装自动分配。
- 空闲作战单位现在执行局部自动警戒；普通 Move 到点后转为空闲警戒，活动采集者和施工 Builder 明确禁止自动攻击，避免经济/施工状态机被战斗意图抢占。6 个没有 AttackMove 或显式攻击命令的士兵完成 16 次自动起手并消灭 3 个目标。
- AttackMove 保存每单位编队槽位作为恢复目标，追击期间保留原移动组身份；交战中的单位不参加目的地换槽、让位和溢出恢复。16 人小队接敌后形成 16/16 唯一终点并全部稳定抵达，不再向一个公共点反复挤压。
- 到点阶段移除 Arrived 单位被通用恢复器反复唤醒的循环。编队达到确定性人数门槛后一次性硬停，并用当前位置做 16 轮有界 Reservation 松弛，既保证最终预约点不重叠，也不要求单位为了预约点继续移动。48 人组 48/48 停稳，连续 240 Tick 最大漂移 `0.00 px`。
- 自动分矿门禁不再只验计数：32/32 Worker 都完成真实“返货→再次出发”循环，收入增加 570；96 Worker/32 矿/4 基地压力场完成 535 次循环、采得 4410、不可达 0。默认可玩局 12/12 Worker 都经历 Going/Gathering/Returning，分散到 10 个矿点，单点最多 2 人。
- 新增 4 个实现无关黑盒：`friendly-building-radial-interaction`、`combat-idle-auto-acquire`、`attack-move-squad-slot-resume`、`group-move-terminal-stability`；同时加强 `economy-auto-patch-distribution` 和 `economy-mass-mining`。123/123 全量业务场景通过，Debug 构建 0 错误、0 警告。
...
### BT：3D Demo 操作、UI、相机与寻路诊断（已完成）

- `Rts3DCameraController` 从场景组合根拆出：方向键/屏幕边缘滚屏、中键拖拽、`Alt+中键` 旋转俯仰、滚轮和触控板手势全部进入同一平滑状态；镜头目标严格限制在模拟 Bounds，录像通过独立 Automation Target 驱动，不再修改手动相机内部字段。
- `Rts3DHud` 只接收不可变显示快照并发出 AttackMove/Stop/Focus/Build/Train/Research/Debug/Return 意图。资源人口、选择摘要、敌方摘要、模式状态、拖框、命令脉冲、建造提示、三行命令卡与按钮可用状态均不读取 Simulation。
- 右下角直接复用解耦 `RtsMinimapControl`；3D 组合根只负责构造 `MinimapSnapshot`，左键聚焦和右键 SmartCommand 仍返回纯模拟坐标。
- 所有建筑统一为 `BoxMesh`，X/Z 尺寸严格等于真实 Bounds，避免圆柱/异形轮廓干扰对矩形边缘交互、绕行净空和卡位问题的判断；功能只由阵营色的轻微明暗/混色区分。
- Presenter 新增缓存式诊断层：选中单位黄色 MoveGoal、青色 SlotTarget、两类目标十字和连线共用一个 `ImmediateMesh`；建造模式复用一个半透明矩形预览盒，正式 PreviewConstruction 成功为绿、失败为红。所有诊断均只读，不修改寻路状态。
- 3D 输入修正目标模式右键语义：Build/AttackMove 中右键只取消，不再取消后顺带下发 SmartCommand；拖框在独立 HUD Overlay 即时显示，正式命令成功后按 Move/Attack/Interact/Build 显示屏幕脉冲。
- 新版 15 秒、451 帧 AV1/WebM 位于 `test_videos/20260714_123248/`，大小 2,788,453 字节；录像自动选中 6 名采集农民并开启 MoveGoal/SlotTarget，覆盖新 HUD、小地图、矩形建筑与诊断表现。

### BU：3D Demo SC2 操作界面与 Rally 专项（已完成）

- 新增 `docs/SC2_3D_INTERFACE_ALIGNMENT.md`，以 Blizzard Buildings/Special Control/Simplified Controls 和公开界面图说明为依据，固定右上资源、底部小地图/选择信息/3×5 命令卡、选择子组、多建筑 Rally、资源自动采集和混合控制组的对齐边界；不复制 Blizzard 美术资产。
- 新增 `Rts3DInterfaceAdapter` 与不可变 UI contracts。Adapter 是唯一读取 Simulation/Catalog 的 UI 边界，HUD 只消费资源、选择、生命、队列、Rally、控制组和命令槽快照并发出 `CommandCardActionSnapshot` 意图；World Presenter 只消费 Rally Marker 快照。
- 3D Rally 已完整暴露：Town Hall/Production 命令卡 `Y` 目标模式、建筑直接右键、地面/资源/友军目标、多建筑统一设置、单位+生产建筑混选右键双语义。资源 Rally 仍由正式 `SetProductionRallyTarget` 与现有出兵/自动采矿流程执行，没有新建玩法分支。
- Presenter 用单个缓存 `ImmediateMesh` 绘制从真实矩形建筑边缘到 Rally 目标的连线、目标环和箭头；友军单位目标实时跟随。中央信息面板显示共同目标或 Mixed target，并聚合生产/研究队列。
- HUD 替换为 SC2 信息架构：右上资源/人口/时间，底部左侧小地图与 0..9 控制组、中间选择子组/生命/队列/Rally、右侧固定 3×5 命令卡；不再显示全知敌军统计。F1、Backspace、Tab、Ctrl/Shift 数字组、Shift 增减和 Ctrl/双击同类型均进入 3D 操作层。
- 新增纯 C# `Rts3DInterfaceSelfTest`，锁定命令槽/快捷键无冲突和矩形 Rally 边缘几何；900 Tick 录制 smoke 额外要求 `rallyMarkers > 0`。新版 15 秒、451 帧 AV1/WebM 位于 `test_videos/20260714_132349/`，录像选择 Town Hall 并设置矿物 Rally。
- Debug/Release 构建均为 0 warning / 0 error；125/125 业务场景与新增 `interface-3d` 模块自检通过。最终 900 Tick 3D smoke 为 `ticks=968, bank=2232/600, enemyFacilities=5, shapes=113, rallyMarkers=1`；全库录像门禁为 167 个 AV1 视频、99 个 manifest、166 个 case 引用。

### BV：真实几何与业务完成条件统一修复（已完成）

- 移动目标和结果已经成为公开、可保存、可回放的数据：地面、单位身体、建筑/资源/交货边缘、攻击/跟随距离、施工/生产让位不再混成一个地面点；真正到达、拥堵就地结束、不可达、目标失效和取消也不再共用 Idle 猜测。
- 单位与建筑的交互、静态碰撞、动态碰撞和扫掠碰撞统一为真实圆形身体对真实矩形边缘。连续施工会逐 Tick 复查接触，返矿、采矿、采气、攻击建筑、跟随和生产出口均消费各自的真实形状与距离。
- Worker 只在前往资源、等待资源、采集中和返货阶段忽略其他单位碰撞，仍然不能穿建筑和地形；普通 Move、施工、战斗等状态保持正常单位碰撞。24/24 采矿穿行矩阵、25/50/65/65 收入曲线、16 Worker + 8 MULE 独立容量和 96 Worker 压力采矿均通过。
- Choke 首次接管增加横向范围与必经排队阶段，修复绕行单位被过早拉向墙角；不对称 40/40、连续波次 48/48、Hold 堵口 24/24 到达，冲突 0。
- 生产 Rally 到友军按双方身体边缘停止；AI 双方返矿边缘 184:0/160:0；可见性场景已完成隐藏→可见→重新隐藏及战争迷雾余量保护，完整回放和热恢复一致。
- 当前导航的 Clearance Bake 已正式重建为 `C14A74B57CF07284`；增量增加/删除障碍与完整烘焙一致，重采样 512/3,080 cells（16.6%）。README 与格式文档已同步。
- 用户已明确同意最后两项增加做法。`combat-attack-building` 现在只对显式攻击建筑，在仍未进入真实射程且追击不可达或连续 90 Tick 没有进展时，按当时存活友军占位重新选择完整矩形边缘；8/8 攻击者实际起手，只发生 2 次重新选择，越界伤害 0。普通 Move、AttackMove 和自动警戒没有接入这条专用恢复。
- 测试框架增加场景明确开启的提前结束：保留最坏截止期限，全部条件完成且最后一条定时操作执行后才结束，并按实际 Tick 回放。`smart-command-gameplay-context` 在 Tick 809 完成、Tick 810 结束，最坏期限仍为 1,194 Tick，Replay 一致。
- 专项录像第一批真实鼠标操作暴露出视觉测试切换模拟后未初始化选择状态。修复放在正式视觉测试入口的初始化流程，不增加越界防御分支；干净重录位于 `test_videos/20260714_203148/`，旧异常日志保留在 `test_videos/20260714_202539/` 作为失败证据。
- 127/127 全量黑盒回归通过；Debug/Release 构建 0 warning / 0 error。全库媒体检查通过 175 个 AV1 视频、101 个 manifest、174 个场景引用。
- 当前温热 Release 基准：256/512/1000 单位移动 P95 `1.879/8.581/18.209ms`，128/256 总单位战斗 P95 `0.775/2.927ms`。1000 单位时间门禁和所有分配门禁仍失败；本专项没有更改阈值，也没有把性能失败写成通过，后续必须作为独立性能任务处理。

BV 功能专项到此收口。详细失败处理、独立计算、用户批准记录和录像位置见 `docs/SEMANTIC_GOAL_RESCUE_PLAN.md`。后续不继续叠加移动或交互启发式；只有新的可复现业务失败才重开，性能超标则进入独立性能任务。

### BW：目标末段全速硬停与停点绝不重叠（已完成）

- 普通开阔地导航不再进入终点减速区。`arrival-hard-stop` 在达到巡航后记录 375 个移动 Tick，主动降速为 0；最后移动速度 `180.000`，下一 Tick 直接为 0，到达后漂移 `0.000000`。
- 圆形单位目标按双方身体边缘和浮点安全余量完成；矩形建筑、资源和交货目标在本 Tick 扫掠线段上二分得到第一个合法接触位置。硬停不会越过交互边界，也不需要靠低速爬行触发。
- 所有终点重排先写临时缓冲区，再核对本批与所有存活单位的停点。任意一对小于 `双方半径 + 2px + 数值容差` 时整批拒绝，绝不提交部分结果或重叠结果。
- 新命令的槽位同时避开其他单位的既有停点、普通静止身体和活动让路点；三秒堵塞就地结束保留原唯一停点，不再直接把当前位置覆盖进去。
- 被挤动单位稳定 6 Tick 且偏离停点超过 8px 后才迁移；连通片区最多 128 个单位，目标距实体最多 12px。失败时保留旧停点且不周期空转，只有实体再次移动才重新尝试。
- 小队提前整体结束若无法生成安全停点，就保留原移动组与原停点并冷却 90 Tick；期间原有换位、让路和外围扩展继续运行，不以拆组换取性能。
- 两批共享目标为 32/32 到达、停点最小净空 2.00px、实体重叠 0、最大偏差 4.7px；外围/内圈和高速超车场景均为 80/80 到达、重叠 0。控制组交叉场景中两个工人最终停点偏差均为 1.0px。
- 128/128 全量黑盒回归通过，Debug/Release 构建 0 warning / 0 error。四段专项 AV1/WebM 位于 `test_videos/20260715_025051/`；全库媒体门禁为 179 个 AV1 视频、102 个 manifest、178 个场景引用。
- 温热 Release 移动 P95 为 `4.486/7.720/27.807ms`，战斗 P95 为 `0.999/2.814ms`。256、1000 单位时间和五档分配门禁未通过；没有放宽预算，性能优化继续作为独立 TODO。

### BX：SC2 风格地形专项 T1 第一纵切（进行中）

- 新增 [SC2 地形对齐研究](SC2_TERRAIN_ALIGNMENT_RESEARCH.md) 与 [地形专项计划](SC2_TERRAIN_IMPLEMENTATION_PLAN.md)，冻结高度层、坡道、路径种类、可建造、视野与表现之间的独立关系；不把贴图直接当玩法规则。
- 纯 C# 新增 `TerrainMapSnapshot`、`ITerrainMapQuery` 与 `TerrainMapBuilder`：支持 `0..15` 悬崖层、表面材质、Ground/Shallow/Deep/Air、Buildable、Ramp、BlocksVision/BlocksCreep、稳定字节和哈希。
- `StaticWorld` 以可选查询接口接入地形；圆形单位位置、线段通过和最终移动约束会阻止跨崖并允许合法坡道。没有传入地形的旧场景保持原平地行为。
- 建筑静态放置读取地形平整度和 Buildable，新增稳定失败码 `TerrainUnbuildable`；浅水可以允许普通地面单位通过，同时拒绝建筑。
- 3D 新增独立 `Rts3DTerrainPresenter`，按材质批量生成地表与崖壁；单位、建筑、资源、弹道、选择圈、命令与 Rally 标记统一抬到地形高度，鼠标改为和实际地表求交。
- 可玩 3D 遭遇战使用程序生成测试材质：Badlands、Rock、Sand、Shallow Water、Metal。既有硬障碍显示为高崖，浅水走廊可走不可建，不使用任何 Blizzard 美术资产。
- `terrain-map` 自测覆盖悬崖阻断、坡道跨层、平地/坡道/浅水建造、Ground/Float 水陆矩阵、三处高度、稳定哈希和 `StaticWorld` 联合查询。Debug 构建 `0 warning / 0 error`；独立 Godot 自测明确打印 PASS。3D 场景 headless 启动 10 帧无运行时错误。

T1 已完成并由地形资源、Clearance Bake 地形哈希、真实坡道通行和 AV1 录像闭环；随后 T2 也已完成，见下节。未经 SC2 实机确认的坡道临界距离没有硬编码进正式规则。

### BY：SC2 风格地形专项 T2 高低地视野与交战（已完成）

- 纯 C# 地形查询负责低看高、连续坡道高度与 Smoke/Undergrowth 内外遮挡；高架观察方式、观察高度均为单位配置，不写死在 Godot 表现层。
- 玩家可见性成为战斗权限的唯一入口；共享视野、探测、Cloak/Burrow 与地形遮挡按各自职责组合。探测不创造越过悬崖的普通视野。
- 合法发射的弹道丢失视野后继续命中；尚未发射或正在持续选敌的攻击在目标退回不可见高地后停止。
- 新增五组真实生产模拟 case 和独立 3D 遭遇战。专项结果覆盖低地拒绝、高架观察、坡道探头、盟友共享、探测组合、烟雾内外、退回高地以及飞行中丢失视野。
- 首帧视野在战斗权限检查前完成初始化，修复“场景生成后立即下令会被错误取消”。六个受影响的既有战斗关卡逐个复验通过。
- Replay Package v30、Hot Snapshot v31、State Hash v32 完整保存新增未来态。Debug 构建为 0 warning / 0 error。
- 完整 `--self-test` 已通过；专项五组地形视野 case、3D 自动演示和 AV1 媒体门禁全部通过。
- `test_videos/20260715_170102/terrain-vision.webm` 为 AV1/WebM、CRF 32、preset 8，383,955 字节；`RTS_TERRAIN_VISION_DEMO_PASS` 和媒体门禁均通过。

地形专项下一项是 T3 数据工作流：Godot Resource/编辑工具需要能刷高度、表面、通行、建造、坡道和视野标记，并在导出前给出带格子坐标的结构错误。T2 不再继续堆叠手感微调。

### BZ：SC2 风格地形专项 T3 数据工作流（已完成）

- 可编辑 `RtsTerrainAuthoringResource` 显式保存尺寸、材质、逐格规则、锚点和检查参数；编辑数据与运行时规范 payload 不再混用。
- 纯 C# 编辑文档提供 Height/Surface/Pathing/Buildable/Vision/Creep/Ramp 正交笔刷；一次拖动只在结束时写回数组，并通过 Godot UndoRedo 形成一个原子操作。
- `RtsTerrainAuthoring2D` 和独立编辑器插件提供十类可组合覆盖层、初始化/正确尺寸重排、检查与导出；UI 只发出编辑意图，不承担玩法判断。
- 导出会拒绝断裂坡道、孤岛、半径净空不足的假通路、出生/资源/目标断连和损坏数组，错误包含稳定码及 `[column,row]`。
- 编辑资产转换为既有不可变 `TerrainMapSnapshot` 后才进入运行时；保存、跨进程加载和稳定哈希已经打通。
- 专项自测和自动工作区演示均通过。操作、边界和命令见 [地形数据编辑与导出工作流](TERRAIN_AUTHORING_WORKFLOW.md)。
- `test_videos/20260715_175336/terrain-authoring.webm` 为 181 帧 AV1/WebM；Manifest 保存 `initial/orthogonal/ramp/rules/export/hash` 全部断言，媒体门禁通过。

下一项固定为 T4：从地形自动提取连通区域、坡道入口和通道宽度，并接入现有高层路线与狭口控制。Clearance Bake 绑定地形哈希和三档地形烘焙已经提前完成，不重复实现；局部避让、推挤和三秒就地结束也不归地形模块处理。

### CA：SC2 风格地形专项 T4a 自动拓扑（已完成）

- 地形与匹配 Clearance Bake 自动生成三档连通区域、坡道入口、真实宽度、尺寸通过掩码和稳定哈希；生产运行不再要求手写所有 Portal。
- 高层规划只输出跨区域坡道口，区域内部继续使用生产 GridPathProvider。两坡道/三高度层正反路线分别稳定输出 `0,1` 与 `1,0`。
- ChokeController 能在离开首个坡道后接管后续坡道；三单位真实运行逐 Tick 观察到两个 Choke，最终 3/3 到达顶层。
- 20px 坡道允许 Small/Medium、拒绝 Large；160px 坡道三档通过。车道额外保留 3px 挤压边，修复最外车道被推出坡道后永久卡住。
- 坡道局部 Bake 必须从低层入口实际连到高层出口；静态岩石完整横封坡道时三档掩码均为 None，高层路线不再产生假通路。
- 既有 TerrainTraversal 生产场景改为自动拓扑：三档各 2 区域、1 个 160px 坡道、路线 2 个入口点/Choke 0，仍保持 18/18 到达、穿崖 0、最小间距 16.00。
- 详细设计和边界见 [地形与高层寻路拓扑](TERRAIN_NAVIGATION_TOPOLOGY.md)。
- 新录像 `test_videos/20260715_184546/terrain-traversal.webm` 为 451 帧 AV1/WebM；自动拓扑、18/18 到达、穿崖 0、浅水禁造和最小间距全部进入 Manifest 门禁。
- 128/128 全量回归通过；双向 16/16、非对称 8/32、连续波次 24/24、Hold 堵口 12/12 全部到达且冲突 0。第一次全量测试暴露的 Hold 误接管没有通过改阈值掩盖，而是增加高层路线双端点归属后修复。

T4a 完成后的下一项是 T4b：局部地形修改的 Chunk 级拓扑更新、建筑/桥梁/可破坏岩石的动态连通变化，以及大型地图平行坡道的路线选择和构建性能门禁。T4 尚未标记整体完成。

### CB：SC2 风格地形专项 T4b1 建筑动态拓扑（已完成）

- 建筑 Hard Commit / 拆除现在会把准确 footprint 送入动态高层拓扑；预览、Ghost 和未来 Shift Build Reservation 不改变移动空间。
- 三档运行时连通快照只在受影响 Clearance Chunk 内重新做碰撞查询，Chunk 外复用旧位图；区域与坡道连接再稳定重标。单次 Large 更新为 4 Chunk、3,072 / 13,824 采样，完整分析 0。
- 双坡道门禁覆盖 Small/Medium/Large/Huge 四档：路线序列稳定为 `0>0>0>1>0>1>0`，即小/中建筑不误封，Large/Huge 封近坡走远坡，移除恢复。
- “有建筑/无建筑”两份热状态反向恢复会显式刷新派生拓扑，路线准确恢复为远坡/近坡；相同 revision 不会导致旧缓存串状态。
- 独立 `TerrainDynamicTopology3D.tscn` 使用真实建筑放置、真实群组 Move 和生产寻路栈演示 `Choke 1 → Choke 0`，没有在表现层伪造路线。
- `test_videos/20260715_195102/terrain-dynamic.webm` 为 601 帧、20.03 秒 AV1/WebM；业务门禁为 3/3+3/3 到达、非法穿崖 0、最小间距 16.00、增量 2、全量 0。
- 128/128 全量回归通过，既有移动、建造、回放、AI、战斗和狭口阈值未放宽。
- 详细合同、性能数字和运行命令见 [地形与高层寻路拓扑](TERRAIN_NAVIGATION_TOPOLOGY.md)。

T4b 剩余：地形格本身的局部修改；可破坏岩石/桥梁状态与回放；大型正式地图的完整拓扑重标和路线查询耗时门禁。建筑摆放链路已经完成。

### CC：T4b2 测试地形预制库（已完成）

- 12 张不同结构的纯 C# 目录地图和 12 份不可变 `.tres` 已生成；不是同布局换皮。每张均有稳定 ID、用途、Tags、起终点、建造探针和可选动态阻挡区。
- 专项门禁完成 12/12 唯一哈希、规范往返、合法锚点、实际连通、建造结果、坡道数量与路线 Choke 数；21 个坡道全部匹配。
- `parallel-ramp-bypass` 证明封近坡仍有备用路，`island-causeway` 证明封唯一陆桥必定断连。
- 预制真实暴露了 `NegativeX/NegativeY` 坡道局部搜索漏边问题；已从算法修复，四个坡道方向统一取两端邻域，不修改测试预期规避。
- 新增可手动浏览和自动录像的 `TerrainPresetGallery3D.tscn`；详细清单见 [测试地形预制库](TERRAIN_TEST_PRESETS.md)。
- `test_videos/20260715_204017/terrain-presets.webm` 为 721 帧、24.03 秒 AV1/WebM；12 资产、12 哈希和 21 坡道全部进入 Manifest 门禁。
- 128/128 全量业务回归通过；四向坡道修复未改变既有门禁阈值。
