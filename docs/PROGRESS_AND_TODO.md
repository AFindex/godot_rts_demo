...
- 113 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
| S11 实际 RTS 玩法 | H3/J2b2b + S8-E5a + SC2-X1/C3 工程闭环完成 | 双资源、建筑/生产/科技、扩张、视野/胜负、双 AI 持续遭遇战、完整操作与战斗、Package v26/Hot v25/Hash v26 | 后续能力由遭遇战关卡暴露的缺口驱动 |
...
C3 的多单位分槽和订单覆盖机制至此不再重写。剩余对齐工作被明确压缩为策略证据：E0 决定 Hold/Harvest/盟友/敌军的最终动作；Detection/Visibility 完成后补隐藏单位无泄露矩阵。P2 内容仍按真实玩法需求触发。
