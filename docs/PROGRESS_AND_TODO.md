...
- 112 个黑盒业务场景通过稳定测试接口驱动，不直接读取路径点、Steering、UnitStore、CombatStore、EconomySystem、ConstructionSystem、ProductionSystem、TechnologySystem 或队列内部数组。
...
| S11 实际 RTS 玩法 | H3/J2b2b + S8-E5a + SC2-X1 完成 | 双资源、建筑/生产/科技、扩张、视野/胜负、双 AI 持续遭遇战、完整操作与战斗、Package v25/Hot v24/Hash v25 | 后续能力由遭遇战关卡暴露的缺口驱动 |
...
X1 与本轮可执行的 P1 对齐项至此收口。后续 TODO 不再是继续微调出口启发式：C2/C3 的 Idle/Hold/Harvest、多单位施工让位和隐藏敌军矩阵等待 E0 实机证据及 Visibility/Detection 支撑；P2 的 Power/Creep/Add-on/Lift、三族建造方法和完整 MULE 内容只在实际玩法需要时开启。默认大型 Player vs AI 闭环与确定性恢复已经满足本轮停止条件。
