...
Replay Package v7 与热快照 v7 已保存采集、建造、生产和建筑战斗目标未来态。Unit Command Log v2 使用独立 TargetBuilding 字段；Production Command Log v3 保存 Rally 目标以及 Train 的完整多建筑前置条件，出生后的 Rally Move/Gather 仍是派生系统命令，不重复写入 Unit/Economy Log。
...
状态 Hash v2～v7 是历史移动至类型化 Rally 边界。当前 Hash v8、Package v7 与热快照 v7 由 77 项全量回归验证。
