...
- `SimulationCommandLogSnapshot` 使用格式版本 2、固定小端序和规范单位 ID 顺序进行序列化。
...
当前记录 `Move / AttackMove / AttackTarget / AttackBuilding / Stop / Hold`，目标单位和目标建筑使用不同字段。Shift 出队、追击、寻路和伤害等派生行为不会重复写入日志。
...
+ TargetUnit + TargetBuilding + UnitCount + sorted UnitIds
```

读取时拒绝不支持的版本、截断数据、非法 Tick、非法命令类型、非有限目标点、空单位集合、乱序或重复 ID，以及尾部多余数据。同一逻辑日志必须生成 byte-identical 的规范字节和相同稳定 Hash。

当前验收夹具记录 7 条命令，规范载荷为 302 字节，日志 Hash 为 `9F84986E50371135`。它同时验证 round-trip、未知版本和截断载荷拒绝。
...
重建时先严格匹配三类资源身份，再按清单创建 Footprint、单位、经济和正式建筑状态，并要求初始状态 Hash 完全一致。每 Tick 固定应用世界、建造、经济、单位命令，然后推进模拟。
...
头部绑定快照格式、状态 Hash 格式、Package Hash、Tick、声明状态 Hash 和模拟容量；正文按逻辑状态保存 Unit、Combat、Economy、Construction、Queue、World、Choke 和私有请求数据。当前 5 工人活跃施工载荷为 2,751 字节；大小随实体与路径状态变化，不作为协议常量。
...
Replay Package v4 与热快照 v4 已保存采集、建造和建筑战斗目标未来态。Unit Command Log v2 使用独立 TargetBuilding 字段；Train/Rally 仍随 S11-D 的生产实体加入。
...
状态 Hash v2/v3/v4 分别是历史的移动、经济和施工边界。当前 Hash v5、Package v4 与热快照 v4 由 71 项全量回归验证；建筑战斗录像位于 `test_videos/20260711_182334/`。
