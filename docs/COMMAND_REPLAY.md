...
Replay Package v3 与热快照 v3 已保存采集和建造未来态。Build 保存已解析的完整 Profile，派生 Footprint 不进入 World Log；Train/Rally 仍随 S11-D 的正式生产实体加入。
...
状态 Hash v2 录像是经济加入前的历史基线；Hash v3 对应经济格式 v2。当前 Hash v4 与建造格式 v3 由 70 项全量回归验证；施工持久化录像位于 `test_videos/20260711_173933/`。
