...
Replay Package v6 与热快照 v6 已保存采集、建造、生产和建筑战斗目标未来态。Unit Command Log v2 使用独立 TargetBuilding 字段；Production Command Log v2 的 Rally 保存目标种类、稳定 ResourceNode/Unit ID 与回退坐标，出生后的 Rally Move/Gather 仍是派生系统命令，不重复写入 Unit/Economy Log。
...
状态 Hash v2/v3/v4/v5/v6 分别是历史的移动、经济、施工、建筑战斗与地面 Rally 边界。当前 Hash v7、Package v6 与热快照 v6 由 76 项全量回归验证。
