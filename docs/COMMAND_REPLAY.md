...
Replay Package v5 与热快照 v5 已保存采集、建造、生产和建筑战斗目标未来态。Unit Command Log v2 使用独立 TargetBuilding 字段；Production Command Log v1 使用独立 Train/Cancel/Rally 语义，出生后的 Rally Move 不重复写入 Unit Log。
...
状态 Hash v2/v3/v4/v5 分别是历史的移动、经济、施工和建筑战斗边界。当前 Hash v6、Package v5 与热快照 v5 由 74 项全量回归验证。
