...
Replay Package v8 与热快照 v8 已保存采集、建造、生产、研究和建筑战斗未来态。Unit Command Log v2 使用独立 TargetBuilding；设施命令日志 v4 保存 Train/Cancel/Rally/Research/CancelResearch 的完整解析 Profile。研究等级、互斥组、队列和进度进入规范运行态。
...
状态 Hash v2～v8 是历史移动至生产前置边界。当前 Hash v9、Package v8 与热快照 v8 由 78 项全量回归验证。
