...
```text
ability_runtime=47/47 unit_bindings=17 building_bindings=6 families=45 prototype=39 missing=0 unclassified=0
```
...
主动拾取/丢弃和死亡掉落尚未进入确定性状态，因此该家族仍保持 blocked。
...
当前二进制版本为 Ability Catalog 20、Unit Command Log 9、Production Command
Log 16、Production Catalog 12、State Hash 44、Hot Snapshot 46、Replay Package 45。
...
与热快照哈希；同时用独立配置样例验证自动施法半径、生命回复、召唤目标反馈伤害及
活动 Buff 的热快照继续运行哈希。第二项严格审计 801 条数据闭包、当前 47 条编译
结果、Ainf/Afbk/AHbn 标量逐字段映射、JSON 法力、召唤单位配置、复合挂点路径、双武器/目标层、
...
manifest 中。机器生成的运行时报告当前确认：53 个已分类家族覆盖 137 条 rawcode；
单位引用家族仍有 240 个未分类。当前人族运行时编译 47 项、45 个家族，其中
39 项有玩法原型编译器。当前 17 条有科技门槛的技能使用 13 个 requirement 对象，
