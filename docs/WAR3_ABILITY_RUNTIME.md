...
当前二进制版本为 Ability Catalog 24、Unit Command Log 9、Production Command
Log 16、Production Catalog 12、State Hash 47、Hot Snapshot 49、Replay Package 48。
...
`Aens/Acri/Ablo/Arej/Afae` 的 15 条跨种族变体；Ainf/Afbk/AHbn/Adef/Aclf/Acmg/
Aply/Ahrp 与新增五家族的标量均逐字段映射，同时验证 JSON 法力、召唤单位配置、复合挂点路径、双武器/目标层、
...
manifest 中。机器生成的运行时报告当前确认：58 个已分类家族覆盖 152 条 rawcode；
单位引用家族仍有 235 个未分类。当前人族运行时编译 47 项、45 个家族，其中
