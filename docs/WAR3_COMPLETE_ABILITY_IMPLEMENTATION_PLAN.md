...
- 运行时覆盖审计确认：58 个已分类家族覆盖全量 152/801 条 rawcode；单位实际
  引用的 285 个家族仍有 235 个未分类。当前人族切片为 43 gameplay、3 delegated、
...
下一批继续按机器报告处理通用被动战斗家族（无敌、抗性皮肤、闪避、致命一击）和
系统委托家族；技能侧攻击附伤与区域 effect 仍需复用同一 Combat Object 查询边界，并为
