...
- 当前 Godot 编译 47 条人族候选、45 个行为家族、17 个单位绑定和 6 个建筑绑定；
  40 条 rawcode（39 个家族）具备玩法编译器，另外条目为委托或纯表现对象。
...
- 运行时覆盖审计确认：62 个已分类家族覆盖全量 163/801 条 rawcode；单位实际
  引用的 285 个家族仍有 231 个未分类。当前人族切片为 43 gameplay、3 delegated、
  1 presentation、0 blocked、0 unclassified。
...
- [已完成] 导出 273 条 ItemData、profile、资源引用与物品-Ability 绑定。
- [已完成] 神秘藏宝室 9 件商品的目录、价格、库存、前置、冷却和效果参数改为配置驱动。
...
1. `Adef/Aclf/Acmg/Aply/AInv/Aihn/Arep` 的最后阻塞项均已按原始 JSON 数据契约落地，
   当前人族 47 个 rawcode、45 个家族已无 `blocked`/`unclassified`。
2. `Arep/Ahrp` 新增内容中立 `Repair` effect；Data A-E 严格映射修理成本、耗时、
   协助建造成本/速率和海上距离，不保存人族数值常量。
3. 持续修理连接 Ability、Construction、Economy 和 Diplomacy：支持建筑/机械单位
   比例扣费与回血、在建建筑追加进度、手动/自动寻找受损友军目标、距离/资源/生命/
   新命令终止，并进入快照与哈希；机械单位成本/时间读取 Unit JSON 修理字段。
4. `--war3-ability-data-self-test` 锁定 43/3/1/0/0 状态矩阵和 40 个已编译 rawcode；
   `--war3-combat-rules-self-test` 验证真实修理、协助建造、取消及热恢复完全一致。

M4 第一批跨种族通用状态法术已完成：

1. `Aens/Acri/Ablo/Arej/Afae` 五个家族的 15 条原始变体全部通过同一内容编译入口，
   不以单位或 rawcode 保存数值；覆盖报告由 137/53 提升为 152/58，单位引用未分类
   家族由 240 降至 235。
2. 诱捕由 JSON 编译普通/英雄持续时间及坠落表现参数，运行时禁行并临时把空中目标
   切到地面攻击层；状态消失后自动恢复原飞行层。
3. 残废、嗜血和生命恢复分别把 JSON 的移动/攻击/伤害倍率、体型系数及总恢复量
   编译为通用 Modifier；恢复速率严格由 `总量 / 持续时间` 计算。
4. 精灵之火支持真实负护甲，并以 Buff 的施法者、目标和持续时间生成跟随单位的共享
   视野源；不存在固定落点或单位名称特判。
5. 数据门禁动态枚举这五个 baseCode 的所有变体并逐字段比对；模拟门禁覆盖落地层、
   禁行、负护甲、显形 Buff，以及热快照恢复后 Hash 完全一致。
6. 自动施法默认状态不再来自家族注册表。每个单位按 Unit JSON 的
   `defaultActiveAbility` 写入绑定；字段使用 baseCode 时也能匹配单位持有的派生 rawcode。
   Orderon/Orderoff 只决定是否提供自动施法开关，不再偷偷决定初始开启状态。

M4 第二批通用被动战斗家族已完成：

1. `Avul/Arsk/AEev/AOcr` 共 11 条导出变体完成分类并都有实际 effect 编译器；
   `Arsk` 三条无数值能力编译为受科技前置控制的内容中立 `Resistant` Aura。全量
   覆盖从 152/58 提升为 163/62，单位引用未分类家族从 235 降至 231。
2. 无敌不是单位白名单：`Avul` 编译为不可驱散的常驻 Aura。单位伤害和建筑伤害入口
   都查询同一 Ability Catalog；带 Avul 的泉水、商店、传送门等建筑不会被战斗或技能
   伤害绕过。
3. 抗性皮肤不发明减伤比例。目标带 `Resistant` 时，持续效果读取能力等级 JSON 的
   `HeroDuration`；仅声明 `NonHero`、未声明 `Hero` 的法术不能选中它，而仅英雄法术
   也不会因此错误接受非英雄单位。
4. 闪避使用 `DataA × 100%` 的独立防御命中项，多来源取最高概率，不复用 Defend 的
   Pierce/Magic 投射偏转。致命一击完整保存 `DataA-E`：概率、倍率、附伤、闪避和
   proc 后不失手，并按稳定 tick/单位/弹道 ID 进行确定性判定。
5. 新战斗字段进入 Ability Catalog 25、State Hash 48、Hot Snapshot 50 和 Replay
   Package 49；目录、活动 Buff、热恢复和回放均按新布局严格拒绝旧版本。
6. 数据门禁实际编译 26 条跨种族变体，并验证 3 条抗性皮肤 profile 和
   20 个真实单位绑定；战斗门禁验证 100% 闪避、100% 暴击、暴击不失手、附加伤害和
   建筑被动无敌，技能门禁验证抗性目标拒绝与 10 秒普通/3 秒英雄时长选择。

下一批继续按机器报告处理系统委托和通用攻击被动家族；技能侧攻击附伤与区域 effect
仍需复用同一 Combat Object 查询边界，并为
地图桥梁与 M5 Item 实体补齐各自所有权。建筑升级实现细节见
`docs/BUILDING_UPGRADE_RUNTIME.md`，战斗规则见 `docs/WAR3_COMBAT_RULES.md`。

本批类别语义同时用 Blizzard 经典资料交叉核对：[Spell Basics](https://classic.battle.net/war3/basics/spellbasics.shtml)
和 [Mountain Giant](https://classic.battle.net/war3/nightelf/units/mountaingiant.shtml)
明确 Resistant Skin 缩短负面法术、免疫部分英雄排除类法术且不降低伤害；
[Demon Hunter](https://classic.battle.net/war3/nightelf/units/demonhunter.shtml) 将 Evasion
描述为避免攻击并承受零伤害；[Blademaster](https://classic.battle.net/war3/orc/units/blademaster.shtml)
确认 Critical Strike 为攻击 proc、三等级分别使用 2/3/4 倍普通伤害。实际概率、倍率、
前置和变体仍只读取本地 1.27a JSON。
