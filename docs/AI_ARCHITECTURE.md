...
首版“能完整经营并结束一局”的策略已经完成。下一段限定为 S11-H2，不无限调参：

- AI 配置/难度数据资源，不把阈值继续散落在代码中。
- 双 AI 错峰自对战、Director 与 Simulation 成对热恢复。
- 防守增援、撤退与目标价值，但不在没有失败录像前优化微操。
- AI 命令 Replay Package 全程重放；仍然禁止回放时重新运行策略。
