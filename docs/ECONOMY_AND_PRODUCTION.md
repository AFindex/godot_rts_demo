...
Town Hall DropOff 保存建筑矩形半尺寸。返矿目标不是建筑中心或预设左右入口，而是“农民当前位置到扩张后建筑矩形边界的最近点”；扩张量包含单位半径与安全余量。投递选择、移动目标和到达判定共享同一几何语义，因此不同尺寸基地都可用，也不会因底层寻路改动而要求测试读取内部路径。

独立黑盒场景 `economy-mass-mining` 使用 4 个 160×120 Command Center、32 片矿和 96 个农民。测试只通过稳定业务门面建造基地、下达 Gather 并读取工人阶段/移动目标，不接触 `EconomySystem`、寻路缓存或 Steering 内部状态。600 Tick 验收结果：96/96 农民覆盖去程、采集和返程，完成 172 次入账循环，总采集 4,032 Minerals；456 次返矿边缘采样全部合法，无不可达单位。自动镜头先总览四片矿区，再逐个巡视基地；AV1/WebM 录像位于 `test_videos/20260713_002852/economy-mass-mining.webm`。
