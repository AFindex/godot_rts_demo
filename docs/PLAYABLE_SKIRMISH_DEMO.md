...
- 初始 12 个 SCV 通过正式批量分配策略自动分散到主矿，再逐个进入正式 Gather 状态；不是按数组索引取模伪造分配。除此之外没有预生成军队、生产设施、科技或扩张。
...
- 右键己方建筑时，非施工单位按各自来向靠近矩形建筑边缘，不再挤向四个固定面中点；普通 Move 到点后进入局部自动警戒。
...
纯 C# `PlayableSkirmishScenarioSelfTest` 在同一场景定义上运行 1800 Tick：地图 3120×1720、78 个资源点、双方 12 个初始农民；12/12 农民都经历 Going/Gathering/Returning 并完成至少一次返货后再次出发，分散到 10 个矿点且单点最多 2 人，玩家余额达到 2748/600，敌方发展到 6 座设施，比赛保持 Running。
