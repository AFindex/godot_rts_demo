...
- `Completed`：记录完成 Tick、胜方联盟和可选的唯一玩家胜者；同 Tick 无存活联盟时为平局。
...
当前规则是：建立过存在后，活跃正式建筑归零即失败。失去 Town Hall 但仍有 Barracks 的玩家继续存活；这同时验证“关键经济能力”和“比赛生命”没有被错误写成同一个布尔值。剩余 Active 玩家全部属于同一联盟时比赛结束；存活盟友同时成为 Victorious，先出局的队友保持 Defeated。
...
当前 Replay Package v28 保存 Tick 0 比赛清单和外交设置；Hot Snapshot v27 保存阶段、参赛者、建立存在标记、失败 Tick、结束 Tick、`WinnerPlayerId` 与 `WinnerAllianceId`；State Hash v28 覆盖全部比赛和外交未来态。载荷读取会严格验证排序、玩家注册、状态组合，以及所有 Victorious 玩家是否属于胜方联盟。
...
`alliance-shared-vision-team-victory` 进一步验证 2v2：两个敌方玩家出局后，两名存活盟友同时 Victorious；多人队伍胜利不伪造唯一玩家胜者，使用 `WinnerAllianceId`。完整边界见 [阵营、外交与共享视野](ALLIANCE_AND_DIPLOMACY.md)。
