...
当前完成边界新增 D0：`UnitPerceptionProfileSnapshot` 独立声明 `None/Cloaked/Burrowed` 与 DetectionRange；视野网格旁维护派生侦测网格。PlayerView、显式攻击、PlayerKnown 放置、Authority 通用反馈和隐蔽目标自动索敌均消费同一感知合同；Burrowed/普通单位接触豁免不影响建筑与地形。
...
X1 至此收口，并满足本计划最后一个可在当时证据下执行的 P1 工作包。本轮 SC2 核心对齐停止继续添加启发式：C2/C3 的 Hold/Harvest/敌军/盟友动作仍等待 E0；后来因真实施工依赖补完的 Visibility/Detection 为 D0，Alliance/Shared Vision 为 D1。其余 P2 只由真实内容需求触发。

### R1：实机反馈稳定化（已完成，2026-07-14）

本包不扩大 SC2 内容范围，只修复玩家实际操作暴露的五个可复现问题：矩形建筑交互被圆形距离替代、Builder 远距离完成施工、远程单位为攻击槽强制重排、单位末段低速爬行、Marine 开火反馈不足。

冻结后的合同如下：

- 矩形建筑的经济与施工交互统一基于 `SimRect.DistanceSquaredTo`；圆形 ArrivalRadius 只保留给没有矩形尺寸的旧 DropOff。
- 施工触发使用建筑表面距离窄带，接近点必须避开世界、其他 Reservation 和资源节点；Refinery 可忽略自己绑定的气矿。
- 远程武器不申请 melee engagement slot；射程内直接停稳开火，射程外追目标中心。近战继续使用唯一接触槽。
- 最终到点保留物理制动响应，但最低接近速度有界，进入 Stop Radius 后速度立即归零。
...
### R2：编队终点、建筑径向交互与自动警戒抢救（已完成，2026-07-14）

本包处理 R1 后实机再次暴露的四项阻塞：建筑 SmartCommand 只走四个面中点、初始农民没有真正批量分矿且返矿接近点被偏移、普通士兵没有空闲警戒且 AttackMove 共用单点、编队抵达后恢复器反复唤醒造成抖动。

冻结后的合同如下：

- 己方建筑 SmartCommand 的非施工单位按各自来向投影到矩形边缘；施工 Reservation 仍使用避障和窄带接近点，两类语义不再共用“四候选面中点”。
- 玩家普通 Move 才使用队形槽；Gather、Return Cargo、Construction Approach 等系统订单精确使用业务系统计算的矩形接近点。
- 普通 Move 途中不索敌，到点转为 Idle Guard；Idle/Stop/AttackMove 可自动获取敌人，Hold 仍只打射程内目标。活动经济 Worker 和施工 Builder 不参与自动警戒。
- AttackMove 的恢复终点是每单位槽位，不是整组公共 Goal。追击中的目标单位退出 Destination Reflow/Yield/Overflow，脱战后回到原槽。
- Arrived 不再由通用距离检查重新激活；编队在绝大多数成员到位、尾部位于有限距离内且没有交战目标时一次性收口。终态 Reservation 从当前站位确定性松弛，随后速度与路径硬归零。

黑盒结果：径向建筑交互 `4/4` 抵达且目标唯一；空闲警戒 `6` 人消灭 `3` 个敌人且显式命令数为 0；AttackMove `16/16` 唯一槽并全部稳定抵达；48 人 Move `48/48` 停稳且 240 Tick 漂移 `0.00 px`；32 Worker 全部完成实际往返，96 Worker 压力场不可达为 0。全量 `123/123` 通过，专项 AV1/WebM 位于 `test_videos/20260714_021546/`。

R2 到此收口。终点人数门槛、Reservation 松弛轮数和自动警戒范围属于当前确定性策略，不继续为了单段录像逐像素调参；只有稳定黑盒复现或基准越界才允许修改。

### R3：通用动态阻塞、单位路权与挤压收口（已完成，2026-07-14）

本包不是“持续增援集结”的单点补丁，而是覆盖 Move、AttackMove、Rally 恢复和其他普通导航腿的通用合同。目标是同时解决两种必需结果：有路权的单位可以真实挤开阻挡者；确实无法进入原预约点的单位不会永久寻路、抖动或反复唤醒。

- `UnitPushPriorityPolicy` 统一 Steering 避让责任和物理穿透修正。Movement Class 高者可推低者；同级移动者可推普通 Idle/Arrived；低级不能推高级，同级不能推 Hold。
- 活动战斗接触保留 `CombatContactPolicy` 的前摇、近战、冷却与移动武器阻力；普通行军优先级不得破坏已经建立的战斗站位。采矿和施工既有碰撞豁免继续生效。
- 通用阻塞窗口先排除足以在窗口内形成有效进展的正常移动者，再检查前方 `96px` 单位走廊并按 Unit ID 每 6 Tick 错峰；连续 180 Tick（60 TPS 下 3 秒）仍无至少 `max(12px, 1.5×radius)` 的有效进展时，当前导航腿在真实站位就地完成。Move 清除移动意图；普通 AttackMove 收敛当前恢复点，战术追击腿则保留原编队槽，脱战后继续原路线；正式 Shift 队列继续推进。
- 被挤开的静止单位先等待 6 Tick 稳定，再批量迁移终态 Reservation；使用预分配缓冲、8 轮有界松弛，并把新预约限制在真实站位 12px 内，避免每 Tick 全局分配和“预约点把已停稳单位重新拖走”。
- 动态阻塞窗口和预约迁移倒计时进入 Hot Snapshot v29 / State Hash v30；Replay Package 保持 v29。新黑盒 `dynamic-blockage-priority-matrix` 覆盖同级、大小级和 Hold 矩阵，`dynamic-blockage-continuous-waves` 覆盖 42 个跨波次 Move/AttackMove/Rally 类到点压力。全量 125/125 通过，专项 AV1/WebM 位于 `test_videos/20260714_035619/`。

R3 到此收口。后续不为个别拥挤画面增加新的恢复器；只有稳定玩家复现、现有黑盒失败或时间性能门禁越界才重开。当前分配门禁反映的是仓库既有基线漂移，已经单列为性能债，不在本包通过放宽阈值掩盖。
