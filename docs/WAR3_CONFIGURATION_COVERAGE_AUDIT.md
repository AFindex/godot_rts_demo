...
审计日期：2026-07-18，最近更新：2026-07-19。结论先行：当前人族运行时切片的
`blocked` 与 `unclassified` 已归零，但不能据此称为“Warcraft III 全量接入完成”。
数据导出层能够完整索引原始对象；全种族、物品和地图对象的玩法/表现仍只覆盖子集。
运行时必须区分“JSON 可读取”和“行为/表现已验收”，不能用 `missing=0` 代替完成度。

## 1. 可量化覆盖

| 范围 | 原始对象/家族 | 已分类 | 当前结论 |
| --- | ---: | ---: | --- |
| Ability 全量 | 801 / 415 | 163 / 62 | 638 个 rawcode 尚未分类 |
| 单位实际引用 | 461 / 285 | 99 / 54 | 231 个引用家族尚未分类 |
| Item Ability | 234 / 129 | 47 / 22 | 187 个物品技能尚未分类 |
| 当前人族运行时 | 47 / 45 | 47 / 45 | 43 gameplay、3 delegated、1 presentation、0 blocked |
...
已接入玩法的 9 件藏宝室物品仍只是 273 件物品中的子集。地面物品、按距离拾取、
主动丢弃、死亡散落及确定性快照已经落地；出售、随机掉落表和其余未分类 Item
Ability 尚未完成，因此不能宣称“所有道具均已接入”。
...
或静默修正，而会让内容导入失败。地面物品实体、按距离拾取、主动丢弃、死亡掉落、
库存/冷却与地面物品确定性快照均已完成，`AInv/Aihn` 已通过玩法验收。

这里保留的代码映射只有“baseCode 对应哪一种通用行为编译器”，例如 Heal、Flare、
Summon。它决定怎样解释 JSON 字段，不保存任何单位/技能的内容数值。

2026-07-19 的首批跨种族闭环新增 `Aens/Acri/Ablo/Arej/Afae` 五个行为编译器，动态
枚举并实际编译其 15 条导出变体。诱捕的落地层、残废/嗜血的实际战斗 Modifier、生命
恢复的总量/持续时间换算、精灵之火的负护甲和跟随目标视野均进入模拟与热快照；数值
只取 Ability JSON。该批将全量覆盖提升为 152/58，单位引用未分类家族降至 235。
自动施法默认状态同时改为逐单位读取 Unit JSON `defaultActiveAbility`，并按 baseCode
兼容派生 rawcode；行为注册表不再为整族硬编码默认开启。

2026-07-19 的第二批跨种族闭环新增 `Avul/Arsk/AEev/AOcr` 四个行为家族，共分类
11 条导出变体，全部都有实际 effect 编译器。`Arsk` 的 3 条对象本来就没有数值字段，
因此编译为受 requirement 控制的 `Resistant` Aura，不能虚构减伤
常量。该批将全量覆盖提升为 163/62，单位引用未分类家族降至 231。

命中链的审计边界也已固定：闪避和 Defend 的投射偏转是不同字段；致命一击的
DataA-E 全部保留，当前 3 条导出变体的 DataC/D/E 虽为零，也仍逐字段进入编译门禁，
防止未来数据非零时静默丢失。抗性皮肤按同一法术的 HeroDuration 缩短负面效果，并
继承英雄目标排除规则；建筑 Avul 由 BuildingAbilityBinding 直接决定被动无敌。
...
- 当前人族切片已无 `blocked`/`unclassified`；3 个委托能力与 1 个纯表现对象仍按
  各自模块边界统计，不能因按钮出现而改写为技能核心效果。
...
- `--ability-self-test`：要求 projectile snapshot、`permanent_summon=True`，以及
  `configured_scalars` 中 `status=True`（诱捕落地/禁行、精灵之火负护甲/显形）；
- `--war3-ability-data-self-test`：要求 `mana_json=True`、`summon_json=True`、
  `attachments_json=True`、`autocast_json=True`、`configured_scalars=True`、
  `extended_families=True`；后者动态
  锁定九个新增家族 26 条变体，逐等级对照原始 JSON；同时锁定 `Arsk` 三条
  无数值 Aura 和 20 个真实 Resistant 单位绑定；
