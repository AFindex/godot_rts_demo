...
本项目在 T2 前已有 Hidden / Explored / Visible，但当时只是按半径刷格子。T2 现已补上低地到高地的方向性遮挡、连续坡道高度和 Smoke/Undergrowth 内外关系；三态战争迷雾仍保留原合同，没有把特殊遮挡混成第四种迷雾状态。

来源：[Blizzard Game Guide: Map Elements](https://news.blizzard.com/en-us/article/4546768/game-guide-map-elements)

### 1.4.1 高地规则必须区分“看见、允许开火、武器结算”

官方地图元素说明能够确认：低地单位在没有高地视野时不能攻击高地单位。SC2 5.0 补丁又说明编辑器数据允许给“对更高悬崖层的地面攻击”配置额外 miss chance，但默认仍保留此前行为。这意味着项目不应凭印象把固定命中率惩罚写死成所有武器的共同规则。

本项目据此冻结三层合同：目标实际可见才可选敌和起手；高架/飞行观察者可以提供跨悬崖视野；一枚已合法发射的确定性弹道随后丢失视野仍继续结算。最后一条是为了让本项目的统一弹道、回放和热恢复拥有明确未来态，不宣称复刻了 SC2 每种武器的全部例外。若以后增加可丢失、散射或需要持续锁定的武器，应以武器数据显式声明，而不是在地形查询里加特殊分支。

来源：[Blizzard Game Guide: Map Elements](https://news.blizzard.com/en-us/article/4546768/game-guide-map-elements)、[StarCraft II 5.0 Patch Notes](https://news.blizzard.com/en-us/article/23482838/starcraft-ii-5-0-patch-notes)
...
├─ 寻路读取：能否通过、坡道跨层、移动类型
├─ 建造读取：是否平整、是否允许建造
├─ 视野读取：连续地表高度、观察高度、遮挡区域
├─ 战斗读取：玩家实际可见性与合法发射后的弹道合同
