...
当前区域目标层支持 GroundUnit、AirUnit、Building。原始列表中的 tree、wall、debris、
item、ward 尚无统一世界实体接口，因而不会被伪装成单位或建筑；后续应由各自实体
模块加入同一稳定目标查询边界。
...
- Gameplay Profile Catalog 2；Production Catalog 11；Production Command Log 14。
- Ability Catalog 16。
- State Hash 39；Replay Package 40；Hot Snapshot 41。

攻击/护甲类型、单位护甲科技、逐武器伤害科技、最小射程、区域半径/目标层、传播
类型/参数/距离科技、Facing/PreviousFacing/TurnRate/AttackHalfAngle，以及飞行中弹道
携带的攻击类型/区域/传播快照都进入规范字节、状态哈希和二进制恢复校验。
...
以及非默认 Production/Godot Resource 二进制往返。
