...
- Worker：Move、Attack Move、Stop、Hold Position，以及 Catalog 中全部 Build 动作。
- CombatUnit：Move、Attack Move、Stop、Hold Position。
- 已完成生产建筑：Set Rally；按 Catalog 和正式 Availability 显示 Train；同类型多选时批量发令、聚合队列，并按每座建筑最新的匹配配方订单取消。
...
J1 完成混合选择、活动子组和快照命令卡。J2a 完成混合 Control Group 与 Alt 抢组。J2b1 完成 Move、Attack Move、Rally 目标模式。J2b2a 完成 Build 类型选择、正式预览与提交。J2b2b 完成同类型多建筑批量生产、取消策略和队列聚合。

表现增强仍可继续做图标资产、快捷键重映射、tooltip 和最终皮肤，但它们不再阻塞玩法闭环。

批量生产语义参考 [Blizzard Buildings Guide](https://news.blizzard.com/en-us/article/4488317/game-guide-buildings) 的同类型生产建筑群组操作：一次 Train 意图作用于活动子组中所有当前可用的同类型建筑。计划按 Building ID 稳定排序；执行仍逐个进入正式生产 API，所以共享资源、人口和队列容量会按同一稳定次序重新验证，资源不足时允许确定性的部分成功。命令卡显示 `ready X/N · queued Q`；取消不会依赖 UI 队列索引，而是对每个生产者撤销最新一条匹配配方订单。
...
- `operation-build-placement-mode`：静态障碍预览红色拒绝且世界不变；合法位置绿色通过、选择最近合格工人并完成 Supply Depot。
- `ProductionGroupPresentationSelfTest`：乱序/重复生产者和订单被稳定规范化；可用生产者计划、总队列和每建筑最新匹配订单取消策略通过。
- `operation-production-group-batch`：3 Barracks 连续两次批量 Train 得到 `[2,2,2]` 队列；批量取消后为 `[1,1,1]`，最终只出生 3 个 Marine。
...
- Build 放置模式 AV1/WebM 位于 `test_videos/20260712_040340/`。
- 多建筑批量生产 AV1/WebM 位于 `test_videos/20260712_114535/`。
