...
- Worker / CombatUnit：Stop、Hold Position。
- 已完成生产建筑：按 Catalog 和正式 Availability 显示 Train；显示并取消当前生产订单。
...
J1 完成混合选择、活动子组、快照命令卡和已有即时动作。J2a 已让 Control Group 保存单位/建筑混合实体，并支持 Alt 覆盖抢组与 Alt+Shift 添加抢组。以下留给 J2b：

- Move、AttackMove、Build、Rally 等需要进入目标选择模式的命令卡动作。
