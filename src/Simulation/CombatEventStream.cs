...
public readonly record struct CombatEventCopyResult(
    int Count,
    ulong LatestSequence,
    int LostEvents);
...
public sealed class CombatEventStream
{
    ...
    public CombatEventCopyResult CopyAfter(
        ulong sequence,
        ref CombatEvent[] destination)
    {
        var latest = LatestSequence;
        if (sequence >= latest)
            return new CombatEventCopyResult(0, latest, 0);
        var oldest = _count == 0 ? latest + 1 : latest - (ulong)_count + 1;
        var requested = sequence + 1;
        var lostCount = requested < oldest ? oldest - requested : 0UL;
        var lost = (int)Math.Min(lostCount, int.MaxValue);
        var first = Math.Max(requested, oldest);
        var count = checked((int)(latest - first + 1));
        if (destination.Length < count)
            Array.Resize(
                ref destination,
                Math.Max(count, Math.Max(16, destination.Length * 2)));
        ...
        return new CombatEventCopyResult(count, latest, lost);
    }
