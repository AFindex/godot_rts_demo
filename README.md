...
S11-G 已完成：版本化比赛清单、建立存在门槛、玩家能力快照、失败/胜利/平局状态和终局命令权限已经闭环，详见 [比赛生命周期](docs/MATCH_LIFECYCLE.md)。当前整体序列化版本为 Replay Package v27、Hot Snapshot v26、State Hash v27。
...
SC2-C2/C3/D0 工程闭环已推进：多单位施工让位、临时订单覆盖与恢复、普通战争迷雾下的 PlayerKnown 无泄露预览，以及正式 Cloak/Burrow/Detection 未侦测/已侦测矩阵已经确定性实现；Hold/Harvest/敌军动作继续按保守策略等待 E0 实机证据。盟友关系与共享视野尚未实现，能力开关、扫描和种族例外仍是后续内容层。
