...
- 经济/建筑/生产 UI 使用不可变 Snapshot；状态 Hash v7 覆盖生产队列、人口预留、出口等待和类型化 Rally 未来态。
...
- 状态 Hash v7 在既有生产状态基础上保存 Rally 的 Ground/ResourceNode/FriendlyUnit 类型、稳定实体 ID 和回退位置。
...
当前机器最近一次 Release 移动基线为 1.77ms、5.76ms 和 9.00ms P95；双方持续 AttackMove 的 128/256 总单位基准为 2.55/3.86ms P95。完整状态 Hash v7 在 1000 单位场景平均约 1.32ms。
...
当前包含 76 个黑盒业务场景，并覆盖双资源经济、正式建筑施工与持久化、生产队列/人口预留/阻塞出口、Ground/资源/友军 Rally 及其回放热恢复、版本化 Building Type 与 Production Catalog Resource、建筑 SmartCommand/追击/受击/摧毁、Gameplay Profile Resource、Clearance Bake、Resource Fresh Load、dirty-chunk Connectivity、群体终点、动态地图、Portal/狭口、AttackMove、操作层、Minimap、Replay Package、checkpoint 和热快照。
...
S11-D3 已完成：生产目录 Resource 与 Rally SmartCommand 均已闭环。Rally 使用纯 C# 类型化目标协议；工人对资源点直接进入 Gather，友军目标按稳定 Unit ID 在出生时解析，失效时使用确定性回退位置。Production Log v2、Replay Package v6、Hot Snapshot v6 与状态 Hash v7 已同步升级。

下一阶段进入 S11-E 的科技前置与生产可用性规则，不继续扩张 Rally 细节。
