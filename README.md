...
- SmartCommand 将地面/友军位置解析为 Move、敌军解析为锁定攻击，资源点按工人/非工人拆分为 Gather/Move，待续建己方建筑由稳定最低 ID 工人接管；A 修饰统一解析为 AttackMove。
...
当前包含 83 个黑盒业务场景，并覆盖 AI Resource/双 AI 自对战/成对热恢复/无 AI 回放、模块化对局 AI、双资源经济、基地扩张/饱和度/工人转场、玩家视野/探索/命令权限、玩家能力/失败/终局、五类正式建筑、生产队列、多建筑生产前置、正式研究等级/取消/互斥、Ground/资源/友军 Rally 及其回放热恢复、版本化数据、建筑战斗、Clearance、动态地图、Portal/狭口、AttackMove、操作层、Replay Package、checkpoint 和热快照。
...
S11-G 已完成：版本化比赛清单、建立存在门槛、玩家能力快照、失败/胜利/平局状态和终局命令权限已经闭环，详见 [比赛生命周期](docs/MATCH_LIFECYCLE.md)。Replay Package/Hot Snapshot 当前为 v11，状态 Hash 为 v12。
...
S11-I1 已完成：资源点与己方建筑进入正式 SmartCommand 目标协议；混合选择按能力拆分，Gather/Resume 正确替换旧工作与单位队列，乱序选择仍稳定选择最低 ID 施工者。跨域 Shift 队列尚未版本化，因此明确拒绝而非静默降级，详见 [操作层](docs/OPERATION_LAYER.md)。
