...
当前机器最近一次 Release 移动基线约为 1.60ms、4.92ms 和 7.65ms P95；双方持续 AttackMove 的 128/256 总单位基准为 2.27/3.93ms P95。完整状态 Hash v6 在 1000 单位场景平均约 1.20ms。
...
当前包含 73 个黑盒业务场景，并覆盖双资源经济、正式建筑施工与持久化、生产队列/人口预留/阻塞出口/Rally、版本化 Building Type Resource、建筑 SmartCommand/追击/受击/摧毁、Gameplay Profile Resource、Clearance Bake、Resource Fresh Load、dirty-chunk Connectivity、群体终点、动态地图、Portal/狭口、AttackMove、操作层、Minimap、Replay Package、checkpoint 和热快照。
...
S11-D1 生产运行时已完成：版本化单位/配方目录、每建筑五格队列、入队人口预留、取消退款、被堵出口等待、确定性出口候选、Rally Move 和生产建筑摧毁清理均已进入纯 C# 模拟。下一段是生产命令日志、Replay Package 和热快照持久化。
