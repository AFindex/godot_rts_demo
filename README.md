...
S11-G 已完成：版本化比赛清单、建立存在门槛、玩家能力快照、失败/胜利/平局状态和终局命令权限已经闭环，详见 [比赛生命周期](docs/MATCH_LIFECYCLE.md)。当前整体序列化版本为 Replay Package v26、Hot Snapshot v25、State Hash v26。
...
SC2-C2/C3 工程闭环已推进：多单位施工让位、临时订单覆盖与恢复，以及普通战争迷雾下的 PlayerKnown 无泄露预览已经确定性实现；Hold/Harvest/敌军动作继续按保守策略等待 E0 实机证据。盟友关系、Cloak/Burrow 与 Detection 尚未实现，不用普通隐藏单位伪装成完整侦测矩阵。
