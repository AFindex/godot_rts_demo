...
- 经济/建筑/生产/研究 UI 使用不可变 Snapshot；State Hash v31 额外覆盖语义化移动目标、目标形状、目标编号和移动结果，并继续覆盖生产、Rally、施工临时撤离、动态阻塞窗口、推挤后预留迁移、配方前置、科技等级、单位/建筑战斗字段、主动隐蔽过渡、武器移动约束、目标锁定、活跃投射物及研究队列未来态。
...
- Replay Package v29、Hot Snapshot v30 与 State Hash v31 保存完整权威未来态，包括语义化移动目标及结果、主动隐蔽过渡、系统派生的施工/生产撤离、动态阻塞/预留迁移、友方 Rally 跟随及目标死亡后的最后位置。
