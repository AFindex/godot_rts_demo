...
2026-07-14 当前温热 Release 结果为：256/512/1000 单位移动 P95 `1.879/8.581/18.209ms`，128/256 总单位持续战斗 P95 `0.775/2.927ms`。256、512 和两档战斗时间通过；1000 单位超过 16.67ms，五档分配也均超过现有预算，因此整体性能门禁为失败。仓库提交的原始结果保存在 `benchmark_results/latest.json`；本专项没有放宽预算或隐藏失败。
...
当前包含 127 个黑盒业务场景，并覆盖启动页/测试中心、语义化移动目标与结果、真实矩形建筑交互、施工接触矩阵、真实精炼厂循环、攻击建筑堵塞后按实时占位恢复、通用动态阻塞优先级、跨波次拥堵稳定、AI Resource/双 AI 自对战/成对热恢复/无 AI 回放、模块化对局 AI、双 AI 持续发展/扩张/科技/交战关卡、双资源经济、普通/MULE 自动分矿、Rally 填空/枯竭重分配/采矿边际收入、显式返还资源、DropOff 失效恢复、采矿穿行碰撞矩阵、基地扩张/饱和度/工人转场、玩家视野/探索/命令权限、主动 Burrow/Unburrow、Cloak/Burrow/Detection、PlayerKnown 施工预览与 Authority 到场重验、外交/共享视野/联盟胜负、五类正式建筑、Reservation/Ghost、到场 Hard Commit、1/8/32 多单位确定性让位、Hold/Gather/Ally/Enemy 阻挡策略矩阵、Build 放置预览与 Shift 连续建造、生产出口软/硬封锁、生产队列、同类多建筑批量生产/聚合队列/逐建筑取消、多建筑生产前置、正式研究等级/取消/互斥、Ground/资源/友军 Rally 及其回放热恢复、版本化数据、建筑战斗、确定性战斗事件流/投射物及解耦表现、远程原地开火、固定/移动武器约束、自动目标评分与稳定锁定、战斗接触优先级、单位/建筑护甲、属性加成、多段攻击和攻防科技、Clearance、动态地图、Portal/狭口、AttackMove、混合编组/目标模式与操作层、Replay Package、checkpoint 和热快照。
