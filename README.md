...
普通矿已拆成“1 个实际采集槽 + 2 个理想分配位 + 等待队列”。批量右键同一矿片会在同基地矿簇内确定性自动分散；`economy-auto-patch-distribution` 用 12/16/24/32 个农民验证八矿均衡分配、等待、Replay 和热恢复。MULE 独立采集通道与收入曲线校准是下一段，不与普通矿槽位混用。
...
S11-G 已完成：版本化比赛清单、建立存在门槛、玩家能力快照、失败/胜利/平局状态和终局命令权限已经闭环，详见 [比赛生命周期](docs/MATCH_LIFECYCLE.md)。Replay Package/Hot Snapshot 当前为 v22，状态 Hash 为 v23。
