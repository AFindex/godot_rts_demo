...
- 经济/建筑 UI 使用不可变 Snapshot；状态 Hash v5 覆盖经济、建筑生命周期和显式 Unit/Building 战斗目标。
...
- 状态 Hash v5 在建筑状态基础上加入显式目标类型、Unit ID、Building ID 和相关命令队列未来态。
...
当前机器最近一次 Release 移动基线约为 1.12ms、4.49ms 和 8.50ms P95；双方持续 AttackMove 的 128/256 总单位基准为 2.00/7.13ms P95。完整状态 Hash v5 在 1000 单位场景平均约 1.32ms。
...
当前包含 71 个黑盒业务场景，并覆盖双资源经济、正式建筑施工与持久化、建筑 SmartCommand/追击/受击/摧毁、Gameplay Profile Resource、Clearance Bake、Resource Fresh Load、dirty-chunk Connectivity、群体终点、动态地图、Portal/狭口、AttackMove、操作层、Minimap、Replay Package、checkpoint 和热快照。
...
S11-C3 战斗层已完成：建筑拥有显式 CombatTargetKind，SmartCommand 可右键攻击敌方建筑，追击目标落在 Footprint 外缘，摧毁沿用建筑生命周期；Godot 可点选己方建筑查看 HP/状态。Replay Package v4、热快照 v4 和命令日志 v2 已覆盖建筑目标。下一段完成 Building Type Godot Resource/编辑器数据工作流，然后进入生产队列。
