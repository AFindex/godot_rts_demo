...
- 经济/建筑/生产/研究 UI 使用不可变 Snapshot；State Hash v30 覆盖生产、Rally、施工临时撤离、动态阻塞窗口、推挤后预留迁移、配方前置、科技等级、单位/建筑战斗字段、主动隐蔽过渡、武器移动约束、目标锁定、活跃投射物及研究队列未来态。
...
- Replay Package v29、Hot Snapshot v29 与 State Hash v30 保存完整权威未来态，包括主动隐蔽过渡、系统派生的施工撤离、动态阻塞/预留迁移、友方 Rally 跟随及目标死亡后的最后位置。
...
```powershell
F:\my_work\Godot_v4.7-stable_mono_win64\Godot_v4.7-stable_mono_win64_console.exe `
  --headless --path . -- --verify-visual-test construction-close-approach
```
...
当前机器本专项 Release 时间门禁结果为：256/512/1000 单位移动 P95 `1.85/5.42/16.18ms`，128/256 总单位持续战斗 P95 `0.82/2.66ms`，均低于上述时间预算。当前分配结果与未修改 HEAD 对照都显著高于旧 `benchmark_results/latest.json`，因此作为独立基线债务保留；本专项没有放宽分配预算。
...
当前包含 125 个黑盒业务场景，并覆盖启动页/测试中心、通用动态阻塞优先级、跨波次拥堵稳定、AI Resource/双 AI 自对战/成对热恢复/无 AI 回放、模块化对局 AI、双 AI 持续发展/扩张/科技/交战关卡、双资源经济、普通/MULE 自动分矿、Rally 填空/枯竭重分配/采矿边际收入、显式返还资源、DropOff 失效恢复、采矿穿行碰撞矩阵、基地扩张/饱和度/工人转场、玩家视野/探索/命令权限、主动 Burrow/Unburrow、Cloak/Burrow/Detection、PlayerKnown 施工预览与 Authority 到场重验、外交/共享视野/联盟胜负、五类正式建筑、矩形边缘交互与近身施工、Reservation/Ghost、到场 Hard Commit、1/8/32 多单位确定性让位、Hold/Gather/Ally/Enemy 阻挡策略矩阵、Build 放置预览与 Shift 连续建造、生产出口软/硬封锁、生产队列、同类多建筑批量生产/聚合队列/逐建筑取消、多建筑生产前置、正式研究等级/取消/互斥、Ground/资源/友军 Rally 及其回放热恢复、版本化数据、建筑战斗、确定性战斗事件流/投射物及解耦表现、远程原地开火、固定/移动武器约束、自动目标评分与稳定锁定、战斗接触优先级、单位/建筑护甲、属性加成、多段攻击和攻防科技、Clearance、动态地图、Portal/狭口、AttackMove、混合编组/目标模式与操作层、Replay Package、checkpoint 和热快照。
