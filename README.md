...
当前机器最近一次 Release 移动基线约为 1.37ms、4.63ms 和 8.60ms P95；1000 单位主要耗时为 Steering，其次为动态碰撞。双方持续 AttackMove 的 128/256 总单位基准为 1.81/5.52ms P95。完整状态 Hash v4 在 1000 单位场景平均约 1.30ms。
...
当前包含 70 个黑盒业务场景，并覆盖双资源经济、正式建筑施工/取消/生命/Refinery、建造命令回放与活跃施工热恢复、Gameplay Profile Resource、Clearance Bake Resource、Resource Fresh Load/Bake-only 提交、dirty-chunk Connectivity、建筑/净空、群体终点、动态地图、Portal/狭口、AttackMove、操作层、Minimap、Replay Package、checkpoint 和持久化热快照。
...
S11-C2 已把正式建筑纳入确定性边界：Build/Cancel/Resume 使用独立版本化日志，Replay Package v3 保存建筑初始清单，热快照 v3 可以直接恢复活跃施工。下一段补建筑作为战斗目标、Building Type Godot Resource 与编辑器数据工作流，之后进入生产队列。
