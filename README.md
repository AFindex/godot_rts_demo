...
当前机器的 Release 移动基线约为 3.67ms、4.99ms 和 10.00ms P95；1000 单位主要耗时为 Steering，其次为动态碰撞。双方持续 AttackMove 的 128/256 总单位基准为 1.95/5.13ms P95。完整状态 Hash v2 在 1000 单位场景平均约 1.25ms。
...
当前包含 62 个黑盒业务场景，并覆盖 Gameplay Profile Resource、Clearance Bake Resource、dirty-chunk 增量 Connectivity、建筑/净空、群体终点、动态地图、Portal/狭口、AttackMove、战斗占位、操作层、选择/相机、Minimap、命令回放、Replay Package、checkpoint 和持久化热快照。
...
移动、动态地图、战斗移动、基础操作和确定性回放已经形成可运行闭环。S9 当前完成 dirty-chunk walkability 增量更新与诊断；下一层只在实际地图规模证明需要时加入边界 component graph，或转向 Resource 热重载/放置差异面板，不继续无边界追加 UI 功能。
