...
当前机器的 Release 基线约为 2.16ms、11.01ms 和 11.14ms P95；1000 单位主要耗时为 Steering，其次为动态碰撞。
...
当前包含 25 个黑盒业务场景：单单位移动、开放场编队、密集编队、对向人流、垂直交叉流、移动命令替换、快速连续改令、跨命令共享目标、Stop、Hold、混合半径、越界目标、动态建筑局部失效、途中放置绕行、完全封路后移除恢复、动态 Portal 改道、单向狭口、平衡双向狭口、非对称双向流量、连续波次、Hold 堵口恢复、临时包围恢复、不可达重试上限和 192 单位压力场景。
...
动态建筑、双向狭口、跨命令槽位和有限卡死恢复已有可运行版本。下一阶段重点是分阶段性能分析、512/1000 单位基准，以及共享路径查询和 GC 审计。Portal/狭口数据仍由 `DemoMapDefinition` 手工定义，还没有 Resource 数据资产和编辑器烘焙工具。战斗、联机和确定性回放暂未实现。
