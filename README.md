...
当前机器的 Release 移动基线约为 1.81ms、4.92ms 和 9.18ms P95；1000 单位主要耗时为 Steering，其次为动态碰撞。双方持续 AttackMove 的 128/256 总单位基准为 3.42/4.70ms P95。完整状态 Hash v3 在 1000 单位场景平均约 1.28ms。
...
当前包含 67 个黑盒业务场景，并覆盖双资源经济/人口/工人采集、Gameplay Profile Resource、Clearance Bake Resource、Resource Fresh Load/差异/Bake-only 提交、文件监听/去抖/有限重试、dirty-chunk 增量 Connectivity、放置前后 Connectivity 差异、建筑/净空、群体终点、动态地图、Portal/狭口、AttackMove、战斗占位、操作层、选择/相机、Minimap、命令回放、Replay Package、checkpoint 和持久化热快照。
...
S9 数据工作流已经闭环，S11-A 双资源经济和真实工人采集循环已经开始运行。下一阶段优先把 Gather/Build/Train/Rally、经济初始状态和生产队列接入 Replay/热快照格式，再推进建造与生产；导航优化只由新玩法实际暴露的问题触发。
