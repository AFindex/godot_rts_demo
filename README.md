...
- 经济 UI 使用不可变 Overview Snapshot；状态 Hash v3 覆盖玩家账本、资源节点、DropOff、工人阶段和携带量。
...
- 状态 Hash v3 在 v2 动态建筑/狭口未来态基础上加入经济账本、节点与工人工作状态。
...
当前机器最近一次 Release 移动基线约为 1.29ms、4.72ms 和 12.66ms P95；1000 单位主要耗时为 Steering，其次为动态碰撞。双方持续 AttackMove 的 128/256 总单位基准为 1.91/5.04ms P95。完整状态 Hash v3 在 1000 单位场景平均约 1.74ms。
...
当前包含 68 个黑盒业务场景，并覆盖双资源经济/人口/工人采集、经济命令回放与活跃采集热恢复、Gameplay Profile Resource、Clearance Bake Resource、Resource Fresh Load/差异/Bake-only 提交、文件监听/去抖/有限重试、dirty-chunk 增量 Connectivity、放置前后 Connectivity 差异、建筑/净空、群体终点、动态地图、Portal/狭口、AttackMove、战斗占位、操作层、选择/相机、Minimap、命令回放、Replay Package、checkpoint 和持久化热快照。
...
S9 数据工作流与 S11-B 经济确定性格式已经闭环。Gather 和 Refinery 状态变化使用独立版本化命令日志，经济初始清单进入 Replay Package v2，活跃工人循环进入热快照 v2。下一阶段推进带双资源成本、施工状态、退款和 Refinery 绑定的正式建造系统；Build/Cancel 随业务语义一起进入现有命令与快照边界。
