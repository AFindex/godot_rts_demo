# Godot 4.7 .NET RTS skirmish demo
...
普通矿已拆成“1 个实际采集槽 + 2 个理想分配位 + 等待队列”。批量右键同一矿片会在同基地矿簇内确定性自动分散；`economy-auto-patch-distribution` 用 12/16/24/32 个农民验证八矿均衡分配、等待、Replay 和热恢复。MULE 使用独立的每矿单槽通道，可与一个普通农民同时采矿，第二个 MULE 等待且不改变普通 16/16 饱和度；对应场景为 `economy-mule-independent-mining`。`economy-assignment-lifecycle` 进一步覆盖 Rally 填空与矿片枯竭收缩，`economy-mining-income-curve` 锁定近矿 1/2/3/4 农民的递减边际收益和远矿距离惩罚；E2 自动分矿阶段已整体收口。

显式 `Return Cargo` 已进入正式命令、命令卡、Shift 队列与确定性回放：携矿改气、携气改矿不会丢货；DropOff 途中失效会改道，全部失效时携货等待并在恢复后续投。对应黑盒场景为 `economy-explicit-return-cargo` 和 `economy-return-cargo-dropoff-loss`。
Shift Build 已进入正式工人工作流：可连续摆放不同尺寸建筑再排回矿；未来项只建立软 Reservation，不提前阻断寻路，轮到时再重验并 Hard Commit。晚期静态失败全退并跳过，动态友军撤离、主动取消 75%、Builder 死亡清理未来项、Replay/热恢复均由 `construction-queued-builds` 锁定。

生产出口与 Rally 边界已收口：12 个稳定候选会优先选择朝向 Rally 的合法出口；友军独占出口会触发确定性软撤离，敌军、建筑和地形保持硬阻挡并进入 `WaitingForExit`。友方单位目标若在出生前死亡则新单位原地待命；出生后死亡则继续前往其最后位置。`production-exit-rally-boundaries` 覆盖四方向、软/硬封口、解除恢复、两种死亡时序、Replay 与热恢复。

施工动态让位已进入有限 C2/C3 工程闭环：`ConstructionBlockerPolicy` 把 Idle/Stop/Move 友军设为可撤离，把 Hold、采集、其他 Builder、其他活动订单和敌军保守设为等待；`ConstructionEvictionPlanner` 为最多 64 个不同半径单位分配稳定、互不重叠的外沿槽位。撤离使用系统临时覆盖层，不清玩家活动 Move 或 Shift 队列，Hard Commit 后恢复原订单。`construction-multi-unit-eviction` 锁定四档尺寸、`1/8/8/32` 占位、Hold 解除、Replay 和热恢复。

施工预览现已拆成 `PlayerKnown` 与 `Authority` 两条边界：己方动态单位属于可延迟清场的软占位，可直接创建 Reservation；当前可见敌军仍返回 `UnitOverlap`；战争迷雾内或已探索但当前不可见的敌军单位、建筑和 Reservation 不改变预览颜色或接受码。Builder 到场后才以完整世界重验，公开视图只返回 `ClearingFriendlyUnits / KnownOccupant / WaitingForClearance`，不会发布隐藏对象 ID、阵营或原因。全局 Connectivity Guard 同样只在 Authority Hard Commit 前运行，避免玩家预览通过全局拓扑侧信道读取隐藏建筑。

这是一个纯 C# 的可玩 RTS 原型。模拟层不依赖 Godot Node/PhysicsBody，Godot 层只负责输入、绘制和 `NavigationServer2D` 路径查询；经济、建造、生产、科技、AI、战斗、迷雾与比赛状态都运行在正式模拟层。
...
- 通用动态阻塞使用 Small/Medium/Large 移动优先级：同级移动单位可挤开普通静止单位，高级可挤开低级，Hold 与活动战斗接触保持保护；确实无法推进的单位在连续 180 Tick 阻塞后就地结束当前导航段，不再无限抖动。
- 被推动的静止单位连续稳定 6 Tick 后才批量迁移全局目标预留，迁移点最多偏离实际站位 12px；持续增援、跨命令共享目标和外圈封锁都保持唯一预留。
...
- SC2 风格地形 T4b1：建筑真正落地/拆除时按受影响 Clearance Chunk 更新三档高层拓扑；放置预览和软 Reservation 不阻路。双平行坡道会在 Large/Huge 封住近坡时改走远坡，拆除后恢复，四档实际 footprint 均有门禁与独立 3D 录像。
- 测试地形预制库提供 12 张稳定运行时资产：平地、单坡、双路绕行、窄/宽坡、三层折返、环形高地、下沉盆地、浅水浅滩、岛屿陆桥、视野山脊、错位峡谷门和 64×40 四路线大图；用途和生成方式见 [测试地形预制库](docs/TERRAIN_TEST_PRESETS.md)。
...
- 经济/建筑/生产/研究 UI 使用不可变 Snapshot；State Hash v32 额外覆盖地形观察高度与 Ground/Elevated 视野方式，并继续覆盖语义化移动目标、生产、Rally、施工撤离、动态阻塞、科技、隐蔽、武器和活跃投射物未来态。
...
- Replay Package v30、Hot Snapshot v31 与 State Hash v32 保存完整权威未来态，包括地形视野方式、语义化移动目标及结果、主动隐蔽过渡、系统派生的施工/生产撤离、动态阻塞/预留迁移、友方 Rally 跟随及目标死亡后的最后位置。
...
```powershell
F:\my_work\Godot_v4.7-stable_mono_win64\Godot_v4.7-stable_mono_win64_console.exe `
  --headless --path . -- --verify-visual-test economy-mass-mining
```
...
2026-07-15 当前温热 Release 结果为：256/512/1000 单位移动 P95 `4.486/7.720/27.807ms`，128/256 总单位持续战斗 P95 `0.999/2.814ms`。512 和两档战斗时间通过；256、1000 单位时间以及五档分配均超过现有预算，因此整体性能门禁为失败。仓库提交的原始结果保存在 `benchmark_results/latest.json`；本专项没有放宽预算或隐藏失败。
...
当前 Bake 格式版本为 1，源导航哈希 `B8441F9F1544B950`，Bake 哈希 `C14A74B57CF07284`，规范载荷 38,215 字节。
...
当前包含 128 个黑盒业务场景，并覆盖启动页/测试中心、语义化移动目标与结果、无阻挡到点全速硬停、真实矩形建筑交互、施工接触矩阵、真实精炼厂循环、攻击建筑堵塞后按实时占位恢复、通用动态阻塞优先级、跨波次拥堵稳定、跨命令绝不重叠的停点预留、AI Resource/双 AI 自对战/成对热恢复/无 AI 回放、模块化对局 AI、双 AI 持续发展/扩张/科技/交战关卡、双资源经济、普通/MULE 自动分矿、Rally 填空/枯竭重分配/采矿边际收入、显式返还资源、DropOff 失效恢复、采矿穿行碰撞矩阵、基地扩张/饱和度/工人转场、玩家视野/探索/命令权限、主动 Burrow/Unburrow、Cloak/Burrow/Detection、PlayerKnown 施工预览与 Authority 到场重验、外交/共享视野/联盟胜负、五类正式建筑、Reservation/Ghost、到场 Hard Commit、1/8/32 多单位确定性让位、Hold/Gather/Ally/Enemy 阻挡策略矩阵、Build 放置预览与 Shift 连续建造、生产出口软/硬封锁、生产队列、同类多建筑批量生产/聚合队列/逐建筑取消、多建筑生产前置、正式研究等级/取消/互斥、Ground/资源/友军 Rally 及其回放热恢复、版本化数据、建筑战斗、确定性战斗事件流/投射物及解耦表现、远程原地开火、固定/移动武器约束、自动目标评分与稳定锁定、战斗接触优先级、单位/建筑护甲、属性加成、多段攻击和攻防科技、Clearance、动态地图、Portal/狭口、AttackMove、混合编组/目标模式与操作层、Replay Package、checkpoint 和热快照。
...
S11-G/D2a 已完成：版本化比赛清单、Own/Ally/Neutral/Enemy 外交、联盟级共享视野与侦测、玩家能力快照、联盟终局权限，以及主动 Burrow/Unburrow 的正式订单、过渡、视野和行动门禁已经闭环，详见 [比赛生命周期](docs/MATCH_LIFECYCLE.md)、[阵营、外交与共享视野](docs/ALLIANCE_AND_DIPLOMACY.md) 与 [主动隐蔽能力合同](docs/ACTIVE_CONCEALMENT.md)。当前整体序列化版本为 Unit Command Log v5、Replay Package v30、Hot Snapshot v31、State Hash v32。
...
SC2-Q1 已完成：Build 进入正式 Shift 队列，支持不同尺寸连续摆放、软 Reservation、逐项重验、失败退款/跳过、动态撤离、Builder 死亡和最终回矿；Construction Log v4 与 Replay Package v24 保证确定性重放。

SC2-X1 已完成：生产出口优先朝 Rally 方向，友军软撤离、敌军/静态硬封口、解除恢复与友方目标出生前/后死亡边界均已进入黑盒门禁；Replay Package v25、Hot Snapshot v24、State Hash v25 精确恢复系统派生 Follow 状态。

SC2-C2/C3/D0/D1 工程闭环已推进：多单位施工让位、临时订单覆盖与恢复、普通战争迷雾下的 PlayerKnown 无泄露预览、Cloak/Burrow/Detection 矩阵，以及 Own/Ally/Neutral/Enemy、共享视野/侦测和 2v2 联盟胜负已经确定性实现。`construction-blocker-policy-matrix` 又锁定 Own Idle 自动让位与 Hold/Gather/Ally/Enemy 保守等待、权限解除、Replay/Hot 精确恢复；该行为是可替换的项目策略。当前机器没有 SC2 客户端，Hold/Harvest/ReturningCargo/盟友等真实动作继续标记 E0 `Unknown`，详见 [SC2 施工占位实验结果](docs/SC2_EXPERIMENT_RESULTS.md)。能力开关、扫描和种族例外仍是后续内容层。
