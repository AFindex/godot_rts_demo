...
当前机器的 Release 移动基线约为 1.42ms、4.65ms 和 9.02ms P95；1000 单位主要耗时为 Steering，其次为动态碰撞。双方持续 AttackMove 的 128/256 总单位基准为 1.78/4.61ms P95。完整状态 Hash v2 在 1000 单位场景平均约 1.33ms。
...
当前包含 63 个黑盒业务场景，并覆盖 Gameplay Profile Resource、Clearance Bake Resource、Resource Fresh Load/差异、dirty-chunk 增量 Connectivity、建筑/净空、群体终点、动态地图、Portal/狭口、AttackMove、战斗占位、操作层、选择/相机、Minimap、命令回放、Replay Package、checkpoint 和持久化热快照。
...
移动、动态地图、战斗移动、基础操作和确定性回放已经形成可运行闭环。S9 当前完成 dirty-chunk 更新以及 Resource Fresh Load/原子差异第一层；下一阶段可做 Bake-only 安全提交、放置差异面板或实际玩法，不继续无边界追加 UI 功能。
