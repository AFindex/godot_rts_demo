...
当前包含 115 个黑盒业务场景，并覆盖启动页/测试中心、AI Resource/双 AI 自对战/成对热恢复/无 AI 回放、模块化对局 AI、双 AI 持续发展/扩张/科技/交战关卡、双资源经济、普通/MULE 自动分矿、Rally 填空/枯竭重分配/采矿边际收入、显式返还资源、DropOff 失效恢复、采矿穿行碰撞矩阵、基地扩张/饱和度/工人转场、玩家视野/探索/命令权限、Cloak/Burrow/Detection、PlayerKnown 施工预览与 Authority 到场重验、玩家能力/失败/终局、五类正式建筑、Reservation/Ghost、到场 Hard Commit、1/8/32 多单位确定性让位、Build 放置预览与 Shift 连续建造、生产出口软/硬封锁、生产队列、同类多建筑批量生产/聚合队列/逐建筑取消、多建筑生产前置、正式研究等级/取消/互斥、Ground/资源/友军 Rally 及其回放热恢复、版本化数据、建筑战斗、确定性战斗事件流/投射物及解耦表现、固定/移动武器约束、自动目标评分与稳定锁定、战斗接触优先级、单位/建筑护甲、属性加成、多段攻击和攻防科技、Clearance、动态地图、Portal/狭口、AttackMove、混合编组/目标模式与操作层、Replay Package、checkpoint 和热快照。
...
SC2-C2/C3/D0/D1 工程闭环已推进：多单位施工让位、临时订单覆盖与恢复、普通战争迷雾下的 PlayerKnown 无泄露预览、Cloak/Burrow/Detection 矩阵，以及 Own/Ally/Neutral/Enemy、共享视野/侦测和 2v2 联盟胜负已经确定性实现。盟友施工占位使用独立 `AuthorityAlly` 并保守等待；Hold/Harvest/敌军/盟友自动让位动作继续等待 E0 实机证据。能力开关、扫描和种族例外仍是后续内容层。
