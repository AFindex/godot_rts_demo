...
当前机器最近一次 Release 移动基线约为 1.48ms、5.21ms 和 9.18ms P95；双方持续 AttackMove 的 128/256 总单位基准为 1.63/4.36ms P95。完整状态 Hash v6 在 1000 单位场景平均约 1.39ms。
...
当前包含 75 个黑盒业务场景，并覆盖双资源经济、正式建筑施工与持久化、生产队列/人口预留/阻塞出口/Rally及其回放热恢复、版本化 Building Type 与 Production Catalog Resource、建筑 SmartCommand/追击/受击/摧毁、Gameplay Profile Resource、Clearance Bake、Resource Fresh Load、dirty-chunk Connectivity、群体终点、动态地图、Portal/狭口、AttackMove、操作层、Minimap、Replay Package、checkpoint 和热快照。
...
S11-D3a 已完成：Unit Type/Production Recipe Godot Resource、Fresh Load、严格转换、稳定 Hash 和资源驱动的三类单位生产均已闭环。下一段只做 Rally SmartCommand 的地面、资源和单位目标协议。
