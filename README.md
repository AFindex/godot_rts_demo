...
- 经济/建筑/生产 UI 使用不可变 Snapshot；状态 Hash v6 覆盖生产队列、人口预留、出口等待和 Rally 未来态。
...
- 状态 Hash v6 在建筑战斗状态基础上加入生产者、订单、进度、出口等待与 Rally 未来态。
...
当前机器最近一次 Release 移动基线约为 3.50ms、5.66ms 和 8.82ms P95；双方持续 AttackMove 的 128/256 总单位基准为 2.39/5.04ms P95。完整状态 Hash v6 在 1000 单位场景平均约 1.22ms。
...
当前包含 74 个黑盒业务场景，并覆盖双资源经济、正式建筑施工与持久化、生产队列/人口预留/阻塞出口/Rally及其回放热恢复、版本化 Building Type Resource、建筑 SmartCommand/追击/受击/摧毁、Gameplay Profile Resource、Clearance Bake、Resource Fresh Load、dirty-chunk Connectivity、群体终点、动态地图、Portal/狭口、AttackMove、操作层、Minimap、Replay Package、checkpoint 和热快照。
...
S11-D2 已完成：独立 Production Command Log、Replay Package v5、热快照 v5 已覆盖 Train/Cancel/Rally、生产中、出口等待和出生后人口账本。下一段是 Unit/Recipe Godot Resource 数据工作流与 Rally SmartCommand。
