...
当前机器最近一次 Release 移动基线约为 1.35ms、4.61ms 和 10.81ms P95；双方持续 AttackMove 的 128/256 总单位基准为 2.91/4.44ms P95。完整状态 Hash v5 在 1000 单位场景平均约 1.52ms。
...
当前包含 72 个黑盒业务场景，并覆盖双资源经济、正式建筑施工与持久化、版本化 Building Type Resource、建筑 SmartCommand/追击/受击/摧毁、Gameplay Profile Resource、Clearance Bake、Resource Fresh Load、dirty-chunk Connectivity、群体终点、动态地图、Portal/狭口、AttackMove、操作层、Minimap、Replay Package、checkpoint 和热快照。
...
S11-C 已完成：建筑拥有显式 CombatTargetKind，SmartCommand 可右键攻击敌方建筑；版本化 Building Type Resource 已覆盖造价、工期、生命、人口、功能和施工策略，并通过纯 C# 快照隔离 Godot。下一阶段进入 S11-D 单位生产队列、人口预留、出生点阻塞恢复和 Rally。
