...
- 经济/建筑/生产 UI 使用不可变 Snapshot；状态 Hash v8 覆盖生产队列、人口预留、出口等待、类型化 Rally 和配方前置未来态。
...
- 状态 Hash v8 在既有生产状态基础上保存 Rally 目标与活动订单的多建筑前置条件。
...
当前机器最近一次 Release 移动基线为 1.64ms、4.71ms 和 11.86ms P95；双方持续 AttackMove 的 128/256 总单位基准为 1.80/4.57ms P95。完整状态 Hash v8 在 1000 单位场景平均约 1.33ms。
...
当前包含 77 个黑盒业务场景，并覆盖双资源经济、正式建筑施工与持久化、生产队列/人口预留/阻塞出口、多建筑生产前置、Ground/资源/友军 Rally 及其回放热恢复、版本化 Building Type 与 Production Catalog Resource、建筑 SmartCommand/追击/受击/摧毁、Gameplay Profile Resource、Clearance Bake、Resource Fresh Load、dirty-chunk Connectivity、群体终点、动态地图、Portal/狭口、AttackMove、操作层、Minimap、Replay Package、checkpoint 和热快照。
...
S11-E1 已完成：Production Catalog v2 支持多个“建筑类型 × 已完成数量”前置条件，生产校验返回 `MissingPrerequisite`，只读 Availability Snapshot 向 Godot 提供逐条件当前值。Production Log v3、Replay Package/Hot Snapshot v7 与状态 Hash v8 已同步升级。

下一段进入 S11-E2：正式研究/升级队列、科技等级与互斥规则。
