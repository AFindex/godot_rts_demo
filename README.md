...
普通矿已拆成“1 个实际采集槽 + 2 个理想分配位 + 等待队列”。批量右键同一矿片会在同基地矿簇内确定性自动分散；`economy-auto-patch-distribution` 用 12/16/24/32 个农民验证八矿均衡分配、等待、Replay 和热恢复。MULE 使用独立的每矿单槽通道，可与一个普通农民同时采矿，第二个 MULE 等待且不改变普通 16/16 饱和度；对应场景为 `economy-mule-independent-mining`。下一段只补 Rally、枯竭和收入曲线门禁。
