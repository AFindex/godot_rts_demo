...
- 经济/建筑/生产/研究 UI 使用不可变 Snapshot；State Hash v26 覆盖生产、Rally、施工临时撤离、配方前置、科技等级、单位/建筑战斗字段、武器移动约束、目标锁定、活跃投射物及研究队列未来态。
...
- Replay Package v26、Hot Snapshot v25 与 State Hash v26 保存完整权威未来态，包括系统派生的施工撤离、友方 Rally 跟随及目标死亡后的最后位置。
...
当前包含 117 个黑盒业务场景，并覆盖启动页/测试中心、AI Resource/双 AI 自对战/成对热恢复/无 AI 回放、模块化对局 AI、双 AI 持续发展/扩张/科技/交战关卡、双资源经济、普通/MULE 自动分矿、Rally 填空/枯竭重分配/采矿边际收入、显式返还资源、DropOff 失效恢复、采矿穿行碰撞矩阵、基地扩张/饱和度/工人转场、玩家视野/探索/命令权限、Cloak/Burrow/Detection、PlayerKnown 施工预览与 Authority 到场重验、外交/共享视野/联盟胜负、五类正式建筑、Reservation/Ghost、到场 Hard Commit、1/8/32 多单位确定性让位、Hold/Gather/Ally/Enemy 阻挡策略矩阵、Build 放置预览与 Shift 连续建造、生产出口软/硬封锁、生产队列、同类多建筑批量生产/聚合队列/逐建筑取消、多建筑生产前置、正式研究等级/取消/互斥、Ground/资源/友军 Rally 及其回放热恢复、版本化数据、建筑战斗、确定性战斗事件流/投射物及解耦表现、固定/移动武器约束、自动目标评分与稳定锁定、战斗接触优先级、单位/建筑护甲、属性加成、多段攻击和攻防科技、Clearance、动态地图、Portal/狭口、AttackMove、混合编组/目标模式与操作层、Replay Package、checkpoint 和热快照。
...
S11-G/D1 已完成：版本化比赛清单、建立存在门槛、Own/Ally/Neutral/Enemy 外交、联盟级共享视野与侦测、玩家能力快照、联盟失败/胜利/平局状态和终局命令权限已经闭环，详见 [比赛生命周期](docs/MATCH_LIFECYCLE.md) 与 [阵营、外交与共享视野](docs/ALLIANCE_AND_DIPLOMACY.md)。当前整体序列化版本为 Replay Package v28、Hot Snapshot v27、State Hash v28。
