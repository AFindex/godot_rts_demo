...
当前包含 79 个黑盒业务场景，并覆盖双资源经济、基地扩张/饱和度/工人转场、五类正式建筑、生产队列、多建筑生产前置、正式研究等级/取消/互斥、Ground/资源/友军 Rally 及其回放热恢复、版本化数据、建筑战斗、Clearance、动态地图、Portal/狭口、AttackMove、操作层、Replay Package、checkpoint 和热快照。
...
S11-F1 已完成：Town Hall 完工/摧毁驱动基地与 DropOff 生命周期；资源按最近有效基地归属，公开快照提供矿气节点数、当前/理想工人数和饱和度；确定性工人转场进入 Economy Command Log v2。Replay Package/Hot Snapshot 为 v9，状态 Hash 为 v10。

下一段进入 S11-F2：所有权与可见业务信息边界，然后实现关键建筑、生产能力与比赛结束状态。
